...
And **`fp-6` → `fp-7`**: `AppServerMarketplaceSourceKind` gained the enum value `zip` — one **HTTP(S) archive** whose **root is the market root** (the manifest sits at the archive root, not inside a wrapper folder). The three official markets (`experts` / `skills` / `connectors`) accordingly moved off this site's `/source/<market>/…` file-per-entry tree and onto three zip archives on ModelScope; the archive's sha256 is the revision (a `HEAD` to the stable URL reads `X-Linked-Etag`), so unchanged content is never re-downloaded. `url` / `github` / `git` / `directory` all remain, and third-party sources can still mirror a whole tree. Still **no methods added or removed**, `48 / 71` unchanged; for the hand-migration of an existing config see §8 of the [Upgrade and migration guide](/en-US/docs/upgrade).
