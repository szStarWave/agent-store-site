#!/usr/bin/env node
/**
 * get-images.mjs — 查询素材库图片列表
 *
 * 调用 images/get 接口，按过滤条件查询账户下的图片素材。
 * 可用于确认 image_id 是否存在、按尺寸筛选图片等。
 *
 * 入参: '<完整参数 JSON>'
 * 必填: account_id
 ...
 *
 * 常用过滤字段:
 *   image_id       — EQUALS / IN（按 ID 查询）
 *   image_width    — EQUALS
 *   image_height   — EQUALS
 *   image_signature— EQUALS（按 MD5 查询）
 *   source_type    — EQUALS（SOURCE_TYPE_LOCAL / SOURCE_TYPE_API 等）
 *   status         — EQUALS（ADSTATUS_NORMAL / ADSTATUS_DELETED）
 *
 * 示例:
 * { "account_id": "123456789" }
 * { "account_id": "123456789", "filtering": [{"field": "image_width", "operator": "EQUALS", "values": ["1280"]}] }
 * { "account_id": "123456789", "filtering": [{"field": "image_id", "operator": "IN", "values": ["111","222"]}] }
 *
 * 输出（成功）: { "list": [...], "page_info": {...} }
 * 输出（失败）: { "success": false, "error": { "message": "..." } }
 *
 * 注意: 不指定 created_time 时默认查询半年内数据
 * 详细规范: references/materials/images-get.md
 */
...
if (!result.success) {
  console.log(JSON.stringify({ success: false, error: { message: `查询图片列表失败: ${result.error?.message}` } }));
  process.exit(1);
}
