#!/usr/bin/env node
/**
 * negativewords-add.mjs — Add negative keywords to an adgroup
 *
 * Calls POST /v3.0/adgroup_negativewords/add
 * Official doc: https://developers.e.qq.com/v3.0/docs/api/adgroup_negativewords/add
 *
 ...
 *   - phrase_negative_words (string[], required): Phrase negative keywords
 *       Each word: 1-20 full-width chars (max 150 bytes), array min 0, max 900
 *   - exact_negative_words (string[], required): Exact negative keywords
 *       Each word: 1-20 full-width chars (max 150 bytes), array min 0, max 900
 *
 * Output (success):
 * {
...
const result = await callApi({
  method: "POST",
  path: "/v3.0/adgroup_negativewords/add",
