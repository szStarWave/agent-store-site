# `ihr-cli staff +archiveList`

[员工档案业务参数](ihr-staff-archive.md)。
...
| 契约项 | 当前行为 | 状态与证据 |
| --- | --- | --- |
| 输入方式 | flags 或 --json/--stdin；输入互斥；metaDataCode 必填并限定固定档案白名单或 SUBSET；page 从 1 开始，pageSize 默认 20、最大 100，各分支负责真实协议转换。 | `ENFORCED`；internal/shortcuts/staff/archive.go；internal/shortcuts/staff/roster_flex_archive_test.go；test/cases/ihr-cli/staff/{archive-readonly,custom-archive-readonly,boundary-validation}.yaml |
| 公共输出差异 | 无响应头差异；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 | `ENFORCED`；Shortcut runtime 与共享契约 |
| 结构化输出 | response 为所选档案分支的分页业务结果；固定档案和自定义子集不混用。 | `ENFORCED`；业务 reference、实现和 focused tests |
...
| 确认方式 | 确认档案类别、筛选范围和当前页。 CLI 不提供 TTY prompt 或 `--yes`。 | `ENFORCED`；本 reference 与 Staff Agent 规则 |
...
- 自动分页：`ENFORCED` 为禁止；page>=1、pageSize 1-100；每次只查一个已确认档案类别，不自动翻页。
