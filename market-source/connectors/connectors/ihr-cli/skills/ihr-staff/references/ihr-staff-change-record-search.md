# `ihr-cli staff change-record +search`

[员工异动记录参数](ihr-staff-process-search.md)。
...
| 契约项 | 当前行为 | 状态与证据 |
| --- | --- | --- |
| 输入方式 | flags 或 --json/--stdin；输入互斥；page/pageSize 1-based，pageSize 默认 20、最大 100；字段、日期和排序白名单。 | `ENFORCED`；internal/shortcuts/staff/change_record.go；internal/shortcuts/staff/lifecycle_contract_test.go；test/cases/ihr-cli/staff/lifecycle-readonly.yaml |
| 公共输出差异 | 无响应头差异；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 | `ENFORCED`；Shortcut runtime 与共享契约 |
| 结构化输出 | response 为异动记录 content/totalElements/totalPages；响应中的 page/rows 当前不作为可靠分页事实。 | `ENFORCED`；业务 reference、实现和 focused tests |
...
| 确认方式 | 确认人员、异动类型、日期范围和当前页。 CLI 不提供 TTY prompt 或 `--yes`。 | `ENFORCED`；本 reference 与 Staff Agent 规则 |
...
- 自动分页：`ENFORCED` 为禁止；page>=1、pageSize 1-100；不自动翻页，调用方以请求页码为准。
