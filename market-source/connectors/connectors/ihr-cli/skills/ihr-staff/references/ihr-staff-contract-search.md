...
| 结构化输出 | response 为员工合同 PAGE_RESULT。 | `ENFORCED`；业务 reference、实现和 focused tests |
...
| 确认方式 | 确认人员/部门、合同状态和当前页。 CLI 不提供 TTY prompt 或 `--yes`。 | `ENFORCED`；本 reference 与 Staff Agent 规则 |
