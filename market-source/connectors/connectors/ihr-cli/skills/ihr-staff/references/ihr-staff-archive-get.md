# `ihr-cli staff +archiveGet`

[员工自定义档案按员工取数](ihr-staff-archive.md)。
...
| 契约项 | 当前行为 | 状态与证据 |
| --- | --- | --- |
| 输入方式 | flags 或 --json/--stdin；输入互斥；staffId、metaDataCode、metaDataType=SUBSET 必填；metaDataCode 最多 5 个；不接受 ENTITY。 | `ENFORCED`；internal/shortcuts/staff/archive.go；internal/shortcuts/staff/roster_flex_archive_test.go；test/cases/ihr-cli/staff/custom-archive-readonly.yaml |
| 公共输出差异 | 无响应头差异；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 | `ENFORCED`；Shortcut runtime 与共享契约 |
| 结构化输出 | response 为指定员工的自定义子集数据；固定档案拒绝，不做跨员工聚合。 | `ENFORCED`；业务 reference、实现和 focused tests |
...
| 确认方式 | 确认单一员工和自定义子集 code；staffId 必须来自已确认候选。 CLI 不提供 TTY prompt 或 `--yes`。 | `ENFORCED`；本 reference 与 Staff Agent 规则 |
...
- 自动分页：`ENFORCED` 为禁止；单员工；最多 5 个子集 code；不枚举员工或自动重试。
