...
| 契约项 | 当前行为 | 状态与证据 |
| --- | --- | --- |
| 输入方式 | --kind 或 --json/--stdin；输入互斥；kind=all/basic/fixed/custom，仅作 CLI 本地过滤，不进入 HTTP 请求。 | `ENFORCED`；internal/shortcuts/staff/flex_meta.go；internal/shortcuts/staff/roster_flex_archive_test.go；test/cases/ihr-cli/staff/flex-meta-readonly.yaml |
| 公共输出差异 | 无响应头差异；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 | `ENFORCED`；Shortcut runtime 与共享契约 |
| 结构化输出 | response 为过滤后的 FlexMetaData LIST。 | `ENFORCED`；业务 reference、实现和 focused tests |
...
| 确认方式 | 用户当前请求明确需要字段发现或档案路由时可执行一次；如果该步骤只是 Agent 推断，先确认。CLI 不提供 TTY prompt 或 `--yes`。 | `ENFORCED`；Core Gate Set 15 与 Staff Agent 规则 |
...
- 自动分页：`ENFORCED` 为禁止；单请求；不分页、不批量、不自动重试。
