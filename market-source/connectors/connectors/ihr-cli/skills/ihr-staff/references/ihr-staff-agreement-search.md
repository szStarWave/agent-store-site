# `ihr-cli staff agreement +search`

[员工合同/协议参数](ihr-staff-contract-agreement.md)。
...
| 契约项 | 当前行为 | 状态与证据 |
| --- | --- | --- |
| 输入方式 | flags 或 --json/--stdin；输入互斥；page 1-based 并转 backend 0-based，pageSize 默认 20、最大 100；部门 ID 和查询字段本地校验。 | `ENFORCED`；internal/shortcuts/staff/records.go；internal/shortcuts/staff/lifecycle_contract_test.go；test/cases/ihr-cli/staff/lifecycle-readonly.yaml |
| 公共输出差异 | 无响应头差异；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 | `ENFORCED`；Shortcut runtime 与共享契约 |
| 结构化输出 | response 为员工协议 PAGE_RESULT。 | `ENFORCED`；业务 reference、实现和 focused tests |
...
| 确认方式 | 确认人员/部门、协议状态和当前页。 CLI 不提供 TTY prompt 或 `--yes`。 | `ENFORCED`；本 reference 与 Staff Agent 规则 |
