# `ihr-cli staff entry +get`

[员工入职详情参数](ihr-staff-entry.md)。
...
| 契约项 | 当前行为 | 状态与证据 |
| --- | --- | --- |
| 输入方式 | --entry-form-id 或 --json/--stdin；输入互斥；entryFormId 必填且必须来自已确认入职候选。 | `ENFORCED`；internal/shortcuts/staff/entry.go；internal/shortcuts/staff/lifecycle_contract_test.go；test/cases/ihr-cli/staff/lifecycle-readonly.yaml |
| 公共输出差异 | 无响应头差异；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 | `ENFORCED`；Shortcut runtime 与共享契约 |
| 结构化输出 | response 为一条入职表单详情 OBJECT。 | `ENFORCED`；业务 reference、实现和 focused tests |
...
| 确认方式 | 候选不唯一时先确认；不得把姓名或 staffId 当 entryFormId。 CLI 不提供 TTY prompt 或 `--yes`。 | `ENFORCED`；本 reference 与 Staff Agent 规则 |
