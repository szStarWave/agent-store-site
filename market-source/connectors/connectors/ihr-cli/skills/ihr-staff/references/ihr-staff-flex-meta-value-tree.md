# `ihr-cli staff +flexMetaValueTree`
...
| 契约项 | 当前行为 | 状态与证据 |
| --- | --- | --- |
| 输入方式 | flags 或 --json/--stdin；输入互斥；codeTypeId 与 groupCode 同时必填。 | `ENFORCED`；internal/shortcuts/staff/flex_meta.go；internal/shortcuts/staff/roster_flex_archive_test.go；test/cases/ihr-cli/staff/flex-meta-readonly.yaml |
| 公共输出差异 | 无响应头差异；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 | `ENFORCED`；Shortcut runtime 与共享契约 |
| 结构化输出 | response 为树形选项结构；value/label 和父子关系保持服务端事实。 | `ENFORCED`；业务 reference、实现和 focused tests |
...
| 确认方式 | 用户当前请求明确需要解析该树形选项源时可执行一次；如果该步骤只是 Agent 推断，先确认。CLI 不提供 TTY prompt 或 `--yes`。 | `ENFORCED`；Core Gate Set 15 与 Staff Agent 规则 |
...
- 自动分页：`ENFORCED` 为禁止；单请求；不递归补查或自动展开额外组。
