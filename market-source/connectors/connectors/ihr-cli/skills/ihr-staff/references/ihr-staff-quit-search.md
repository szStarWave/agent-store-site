...
| 契约项 | 当前行为 | 状态与证据 |
| --- | --- | --- |
| 输入方式 | flags 或 --json/--stdin；输入互斥；state 默认 pending；page/pageSize 均 1-based，pageSize 默认 20、最大 100；离职状态和 queryParam 走白名单。 | `ENFORCED`；internal/shortcuts/staff/process_search.go；internal/shortcuts/staff/process_search_test.go；test/cases/ihr-cli/staff/lifecycle-readonly.yaml |
| 公共输出差异 | 无响应头差异；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 | `ENFORCED`；Shortcut runtime 与共享契约 |
| 结构化输出 | response 为离职流程 PAGE_RESULT。 | `ENFORCED`；业务 reference、实现和 focused tests |
...
| 确认方式 | 确认人员、离职状态和当前页。 CLI 不提供 TTY prompt 或 `--yes`。 | `ENFORCED`；本 reference 与 Staff Agent 规则 |
