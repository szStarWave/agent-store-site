# `ihr-cli staff +flexMetaSortableFields`
...
| 结构化输出 | response 为该元数据的可排序字段 LIST。 | `ENFORCED`；业务 reference、实现和 focused tests |
...
| 确认方式 | 用户当前请求明确需要排序字段且已有真实 metadata ID 时可执行一次；如果该步骤只是 Agent 推断，先确认。CLI 不提供 TTY prompt 或 `--yes`。 | `ENFORCED`；Core Gate Set 15 与 Staff Agent 规则 |
...
- 自动分页：`ENFORCED` 为禁止；单请求；不分页、不枚举 ID。
