# `ihr-cli smart-kpi appraiser +staff-overview`

员工绩效评估人总览。完整筛选字段见 [SMART-KPI 报表查询](ihr-smart-kpi-reports.md)。
...
| 契约项 | 当前行为 | 状态与证据 |
| --- | --- | --- |
| 输入方式 | 分项 flags 或 `--json/--stdin`；输入互斥；使用员工字段集，空对象在 Runtime 可执行但 Agent 必须先确认范围；拒绝身份、内部链路 ID、分页和未知字段。 | `ENFORCED`；`internal/shortcuts/smartkpi/{shortcuts,query}.go`、`shortcuts_test.go`、CLI boundary cases |
| 公共输出差异 | 无响应头差异；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 | `ENFORCED`；Shortcut runtime 与共享契约 |
| 结构化输出 | `response.tasks[].subjects[].nodes[].appraisers[]`；同节点同评估人只保留一条；维度总评固定读取 `appraisers[].scores[]` 的 `dimensionName/finalScore/kpiGradeSettingName/generalComments`，不读取评估人顶层 `totalScore`。 | `ENFORCED`；`internal/shortcuts/smartkpi/{orchestration,output}.go`、`output_test.go`、live fixture verifier |
...
| 错误与恢复 | `RESULT_LIMIT_EXCEEDED` 只读取范围和恢复动作；任务范围超限且员工等对象条件尚未执行时，只提示补充任务名称、考核周期或任务状态，不展示候选任务数量、单次任务上限，也不得声称该员工关联、匹配或命中了当前任务范围。不得复制错误 JSON、协议字段名、错误码或内部枚举。鉴权错误重新登录；任何远端/结构错误停止，不输出部分结果。 | `ENFORCED`；orchestration、`security_chain_test.go` 与 presentation reference |
...
- 批量执行：`ENFORCED`；最多 10 个任务、20 个员工；每对象最多 50 节点、全命令最多 200 节点。
