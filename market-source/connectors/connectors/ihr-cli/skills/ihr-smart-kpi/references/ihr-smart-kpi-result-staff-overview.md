# `ihr-cli smart-kpi result +staff-overview`

员工绩效结果总览。完整筛选字段见 [SMART-KPI 报表查询](ihr-smart-kpi-reports.md)。
...
| 结构化输出 | response.tasks[].subjects[]；空任务返回成功空集合。 | `ENFORCED`；`internal/shortcuts/smartkpi/{orchestration,output}.go`、live empty-result cases |
...
- 批量执行：`ENFORCED`；最多 10 个任务、200 个被考核员工。
...
- SEC-001：`PASS`（任务查询由后端功能权限和当前用户可管理任务范围约束；被考核对象查询的 taskId 只能来自前一步任务响应，用户输入内部 ID 会被本地拒绝；真实 HTTP 编排测试已验证顺序和 ID 传递）
