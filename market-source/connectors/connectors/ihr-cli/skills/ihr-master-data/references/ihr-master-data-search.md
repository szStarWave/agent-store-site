# `ihr-cli master-data +search`

[主数据查询参数与候选语义](ihr-master-data-lookup.md#search)。
...
| 契约项 | 当前行为 | 状态与证据 |
| --- | --- | --- |
| 输入方式 | 分项 flags 或 --json/--stdin；输入互斥；type 与 keyword 必填；limit 默认 20 且必须大于 0；permissionCode 只来自目标业务契约。 | `ENFORCED`；internal/shortcuts/masterdata/service.go；internal/shortcuts/masterdata/service_test.go；test/cases/ihr-cli/master-data/resolver-readonly.yaml |
| 公共输出差异 | 无额外响应头行为；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 | `ENFORCED`；`internal/shortcut/runtime.go`、共享契约 |
| 结构化输出 | response 包含 type/candidates/count；零候选是成功空结果，多候选由 Agent 消歧，不自动选第一条。 | `ENFORCED`；业务 reference 与 focused tests |
...
| 确认方式 | 只有用户当前请求已经明确需要名称/编码转 ID，或已经确认把主数据解析作为业务查询步骤时才执行一次；否则先确认。CLI 无 TTY prompt 或 `--yes`。 | `ENFORCED`；Core Gate Set 15 与 `skills/ihr-master-data/SKILL.md` |
...
- 自动分页：`N/A`；单请求；不自动扩大 limit、不自动重试。
