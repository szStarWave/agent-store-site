# `ihr-cli master-data +get`

[主数据查询参数与单条语义](ihr-master-data-lookup.md#get)。
...
| 契约项 | 当前行为 | 状态与证据 |
| --- | --- | --- |
| 输入方式 | 分项 flags 或 --json/--stdin；输入互斥；type 与 id 必填；DEPARTMENT ID 规范化为整数；permissionCode 只来自目标业务契约。 | `ENFORCED`；internal/shortcuts/masterdata/service.go；internal/shortcuts/masterdata/service_test.go；test/cases/ihr-cli/master-data/resolver-readonly.yaml |
| 公共输出差异 | 无额外响应头行为；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 | `ENFORCED`；`internal/shortcut/runtime.go`、共享契约 |
| 结构化输出 | response 为一条规范化主数据记录；未找到、权限不足或上游失败使用稳定 master-data error。 | `ENFORCED`；业务 reference 与 focused tests |
...
| 确认方式 | 只有用户当前请求已经明确需要解析该 ID，或已经确认把主数据解析作为业务查询步骤时才执行一次；否则先确认。CLI 无 TTY prompt 或 `--yes`。 | `ENFORCED`；Core Gate Set 15 与 `skills/ihr-master-data/SKILL.md` |
