...
| 契约项 | 当前行为 | 状态与证据 |
| --- | --- | --- |
| 输入方式 | flags 或 --json/--stdin；输入互斥；groupCode/category 条件必填，tab/status 归一为 tabCode；月份使用 yyyy-MM 且开始月份不得晚于结束月份；不接收列表分页 flags。 | `ENFORCED`；internal/shortcuts/workflow/ledger.go；internal/shortcuts/workflow/ledger_test.go；test/cases/ihr-cli/workflow/ledger-readonly.yaml |
| 公共输出差异 | 无响应头差异；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 | `ENFORCED`；Shortcut runtime 与共享契约 |
| 结构化输出 | response 包含 summary/totalElements/groupCode/tabCode/sourceApi；空结果以 totalElements=0 成功返回。 | `ENFORCED`；业务 reference 与 ledger tests |
...
| 确认方式 | 确认审批大类/小类、状态和部门范围；部门名称先用 master-data 按 permissionCode 消歧。 CLI 无 TTY prompt 或 `--yes`。 | `ENFORCED`；Workflow Skill 与业务规则 |
...
- 自动分页：`ENFORCED` 为禁止；单次计数请求；不分页、不拆成多次查询。
