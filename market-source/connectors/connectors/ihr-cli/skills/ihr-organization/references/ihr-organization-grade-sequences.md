# `ihr-cli organization +gradeSequences`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `--criteria` | JSON 字符串 | OPTIONAL | 不发送 | JSON 数组或对象 | 无 | `criteria` | 职级序列过滤条件。 |
...
- `response` 是 `LIST`，不承诺分页统计；项字段以实际返回为准，响应完整度为 `PARTIAL`。
