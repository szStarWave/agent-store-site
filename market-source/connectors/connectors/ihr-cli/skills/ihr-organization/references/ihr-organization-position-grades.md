# `ihr-cli organization +positionGrades`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `--criteria` | JSON 字符串 | OPTIONAL | 不发送 | JSON 数组或对象 | 无 | `criteria` | 职级分组过滤条件。 |
...
- `response` 是 `LIST`；稳定字段包括分组 `id`、`groupName`、`status` 和嵌套的 `positionGradeVos`。响应完整度为 `PARTIAL`。
- 嵌套职级通常含 `id`、`positionGradeName`、`groupName`；raw ID/code 不自动格式化。空结果为 `[]`。
...
| 契约项 | 公开行为 |
| --- | --- |
| 输入方式 | flags 或 --json/--stdin；输入互斥；criteria/orders/page/size 为公开兼容 carrier，结果为 LIST。 |
| 公共输出差异 | 无额外响应头行为；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 |
| 结构化输出 | response 为职级分组 LIST。 |
| 退出码 | 成功、help 和成功 dry-run 为 `0`；本地参数/JSON/范围校验为 `2`；stdin I/O、鉴权、配置、网络、HTTP、业务和输出文件失败为 `1`。 |
| 确认方式 | 确认筛选条件；不得承诺分页。 CLI 不提供 TTY prompt 或 `--yes`。 |
