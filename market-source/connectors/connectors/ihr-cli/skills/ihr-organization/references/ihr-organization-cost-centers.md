# `ihr-cli organization +costCenters`

`--criteria`/`--orders` 与 `--json`/`--stdin` 互斥；JSON 模式提交下表“公开 JSON 映射”列中的字段。条件内容必须是合法 JSON。

## 公开输入

| 参数 | 类型 | 必填状态 | 默认值 | 枚举/格式/单位 | 条件依赖 | 公开 JSON 映射 | 业务说明 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `--criteria` | JSON 字符串 | OPTIONAL | 不发送 | JSON 数组或对象 | 无 | `criteria` | 成本中心过滤条件。 |
| `--orders` | JSON 字符串 | OPTIONAL | 不发送 | JSON 数组或对象 | 无 | `orders` | 成本中心排序条件。 |
...
- 列表稳定字段包括 `id`、`costName`、`costCode`、`isValid`；成本中心 ID/code 保持 raw 值。
- 响应完整度为 `PARTIAL`；空结果为 `list=[]`。不要根据返回字段自动扩大条件、翻页或执行写入操作。
...
| 契约项 | 公开行为 |
| --- | --- |
| 输入方式 | flags 或 --json/--stdin；输入互斥；criteria/orders 为公开 JSON carrier；page 从 1 开始，size 默认 20、最大 100。 |
| 公共输出差异 | 无额外响应头行为；沿用 Shortcut 的 `--pretty/--output-file`，不支持 `--include`。 |
| 结构化输出 | response 为成本中心 PAGE_RESULT。 |
| 退出码 | 成功、help 和成功 dry-run 为 `0`；本地参数/JSON/范围校验为 `2`；stdin I/O、鉴权、配置、网络、HTTP、业务和输出文件失败为 `1`。 |
| 确认方式 | 确认筛选和当前页；criteria/orders 只使用已知业务字段。 CLI 不提供 TTY prompt 或 `--yes`。 |
