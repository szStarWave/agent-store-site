# `ihr-cli organization +jobTitles`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `--criteria` | JSON 字符串 | OPTIONAL | 不发送 | JSON 数组或对象 | 无 | `criteria` | 职务过滤条件。 |
...
- 列表稳定字段包括 `id`、`jobTitleName`、`jobTitleCode`、`jobCategoryId`、`minPositionGradeId`、`maxPositionGradeId`、`effectiveDate`；raw ID/code 保持不变。
- 响应完整度为 `PARTIAL`；空结果为 `list=[]`。
