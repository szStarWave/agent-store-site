# `ihr-cli organization +costCenterGroups`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 无 | 无 | 无 | 无 | 无 | 无 | 无 | 查询当前租户可见的成本中心分组。 |

## 返回契约

- 外层是共享 `success`、`command`、`request`、`response` envelope。
- `response` 是 `LIST`；每项字段以实际返回为准，响应完整度为 `PARTIAL`。
- 名称、编码和 ID 都是 raw 业务数据；空结果为 `[]`，不能把空结果当作权限或登录成功的证明。
