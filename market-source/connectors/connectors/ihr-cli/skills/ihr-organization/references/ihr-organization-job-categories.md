# `ihr-cli organization +jobCategories`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 无 | 无 | 无 | 无 | 无 | 无 | 无 | 查询当前租户可见的职务类型。 |

## 返回契约

- 外层是共享 `success`、`command`、`request`、`response` envelope。
- `response` 是 `LIST`；稳定字段通常包括 `id`、`jobCategoryName`、`jobCategoryCode`，响应完整度为 `PARTIAL`。
