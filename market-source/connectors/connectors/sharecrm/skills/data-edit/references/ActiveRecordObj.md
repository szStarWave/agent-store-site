...
当目标对象为 `ActiveRecordObj`，且需要填写或修改“关联业务模块”/`related_object`/`what_list` 类型字段时，不要写入 `related_object`，也不要使用 `{ObjectApiName: ["id"]}` 简写；必须同时写入 `related_object_data` 和 `related_api_names`。
