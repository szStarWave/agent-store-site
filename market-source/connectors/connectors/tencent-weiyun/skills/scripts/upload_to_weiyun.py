...
鉴权方式（二选一，优先使用 access_token）：
  方式一：OAuth access_token（通过 --access_token 参数或 WEIYUN_ACCESS_TOKEN 环境变量）
  方式二：MCP token（通过 --token 参数或 WEIYUN_MCP_TOKEN 环境变量）

环境变量（可选，也可通过命令行参数传入）：
  WEIYUN_MCP_URL       — MCP 服务地址，默认 https://www.weiyun.com/api/v3/mcpserver
  WEIYUN_MCP_TOKEN     — MCP token（鉴权方式二）
  WEIYUN_ACCESS_TOKEN  — OAuth access_token（鉴权方式一）
  WEIYUN_ENV_ID        — 环境标识（如 sit-xxxxxxxx），可选

示例：
  # 使用 OAuth access_token（推荐）
  python3 upload_to_weiyun.py /tmp/Test666.json --access_token <your_access_token>

  # 使用 MCP token
  python3 upload_to_weiyun.py /tmp/Test666.json --token <your_mcp_token>

  # 使用环境变量
  export WEIYUN_ACCESS_TOKEN=<your_access_token>
  python3 upload_to_weiyun.py /tmp/Test666.json

  # 指定上传目录
  python3 upload_to_weiyun.py /tmp/Test666.json --access_token <your_access_token> --pdir_key <dir_key>
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例：
  %(prog)s /tmp/Test666.json --access_token <your_access_token>
  %(prog)s /tmp/Test666.json --token <your_mcp_token>
  %(prog)s /tmp/Test666.json --access_token <your_access_token> --pdir_key <dir_key>
  %(prog)s /tmp/Test666.json --token <your_mcp_token> --env_id <env_id>

环境变量：
  WEIYUN_MCP_URL       MCP 服务地址（默认 https://www.weiyun.com/api/v3/mcpserver）
  WEIYUN_MCP_TOKEN     MCP token（鉴权方式二）
  WEIYUN_ACCESS_TOKEN  OAuth access_token（鉴权方式一，优先）
  WEIYUN_ENV_ID        环境标识
""",
    )
    ...
    parser.add_argument("--access_token", default=None,
                        help="OAuth access_token（也可通过 WEIYUN_ACCESS_TOKEN 环境变量设置）")
    ...
    parser.add_argument("--env_id", default=None,
                        help="环境标识（如 sit-xxxxxxxx，也可通过 WEIYUN_ENV_ID 环境变量设置）")
    ...
    access_token = args.access_token or os.environ.get("WEIYUN_ACCESS_TOKEN")
    env_id = args.env_id or os.environ.get("WEIYUN_ENV_ID")
    if not mcp_token and not access_token:
        print("错误：必须提供鉴权凭证，二选一：")
        print("  方式一：--access_token 参数或 WEIYUN_ACCESS_TOKEN 环境变量（OAuth）")
        print("  方式二：--token 参数或 WEIYUN_MCP_TOKEN 环境变量（MCP token）")
        sys.exit(1)
    # 构建 HTTP 请求头（优先使用 access_token）
    headers = {
    ...
    if access_token:
        ...
        headers["WyHeader"] = f"mcp_token={mcp_token}"
    if env_id:
    ...
    if access_token:
        print(f"鉴权方式: OAuth access_token")
        print(f"Token:    {access_token[:8]}...{access_token[-4:]}")
    else:
        print(f"鉴权方式: MCP token")
