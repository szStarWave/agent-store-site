...
> **本 skill 内的权威文件**：`references/resume-module-config.json`（原样保留，禁止手改）。
>
...
```json
{
  ...
  "moduleName": "基本信息",
  "hidden": false,          // 该 resumeType 下模块是否隐藏 → 解析后映射为模块 moduleStatus
  "sort": 1,                // 报告输出的模块排序依据
  "dataFieldList": [
    {
      "dataFieldCode": "personalName",
      "dataFieldName": "姓名",     // NO_FILL 话术【{dataFieldName}】的来源
      "showOnPage": true,          // 该字段是否在页面展示 → 参与字段 dataFieldStatus（SHOW/HIDDEN）判定
      "canHidden": false
    }
  ]
}
...
`moduleStatus` / `dataFieldStatus` 由解析阶段赋值，完整规则见 `references/analysis-resume-rules.md`。要点：
...
|---|---|---|
| `noInternshipExperience` / `havingInternshipExperience` / `havingWorkExperience` | 16 | — |
| `onlineApplication` | 18 | — |

> `familyMember` / `language` / `certificate` / `softwareOperate` / `paper` / `creation` 已从配置删除。专业能力评分：非网申类型只可能命中 `professionalSkillsNonOnline` 的 3 个综合字段；网申类型命中 `professionalSkills` 模块字段。

## 4. 评分/诊断引用的关键 moduleCode / fieldCode

关键 moduleCode / fieldCode 常量：
...
字段：
position（应聘岗位）/ qualification（学历）
