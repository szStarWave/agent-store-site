...
> 本文件定义"用户简历原文 → 模块/字段记录集"的转换规则。诊断与评分的输入（可见字段）直接依赖这里产出的 `moduleStatus` / `dataFieldStatus`。
>
...
```
解析流程（由 finalize 子命令确定性执行；模型只负责第 1 步填值）:
  1. 模型按模板逐字段填值 → 产出"模块→记录数组"（见 §10）
  2. idPhoto = 上传头像 URL（见 §2.2）
  3. 专业技能（网申版 professionalSkills）多条合并为 1 条（见 §2）
  4. 专业技能（非网申版 professionalSkillsNonOnline）多条合并为 1 条（见 §2）
  5. 教育经历前缀清洗（见 §2）
  6. 遍历 moduleConfig[resumeTypeCode]（JSON 数组顺序）逐模块处理：
     - 模块无解析内容 → 走"空模块分支"（见 §3）
     - 模块有解析内容 → 走"有内容分支"（见 §4）
  7. 模块排序：显示模块在前 + 隐藏模块在后，sort 重新编号 1..N
```
...
| 清洗 | 规则 |
|---|---|
| 专业技能合并（professionalSkills，网申版） | 多条记录合并为 **1 条**：`languageClassify` / `certificateName` / `softwareName` 用 `、` 拼接并去掉 `\n`；`languageDesc` / `certificateDesc` / `softwareDesc` 用 `\n` 拼接 |
| 专业技能合并（professionalSkillsNonOnline） | 多条合并为 **1 条**：`languageSkill` / `softwareOperation` 用 `、` 拼接去 `\n`；`qualificationCertificate` 用 `\n` 拼接 |
| 教育经历前缀清洗 | `educationExperienceDesc` 用正则 `专业课程:|专业课程：|专业课程` 做首个匹配去前缀 |
| idPhoto 赋值 | `baseInformation.idPhoto` = 上传头像 URL |

> 影响：专业技能两个模块永远只有 **1 条** dataId 记录。

### 2.1 枚举值归一化（解析侧必须执行）

规范解析器会把学历等枚举字段输出为**标准选项值**，入库的 `qualification` 是「大学本科」这类枚举原文，而不是简历里写的「本科」。解析自由文本时必须做同样的归一化，否则评分精确匹配必然落空（"本科" ≠ "大学本科" → 学习能力恒 0）：
...
> 同理，其他有固定选项的字段（政治面貌等）解析时也应输出规范选项值；这些字段不参与评分，但影响 NO_FILL 判定的"有值/空白"结论。

### 2.2 idPhoto（证件照）特殊规则

`idPhoto` 不来自简历文本，而是**用户头像 URL**——只要用户在 App 里传过头像，`idPhoto` 就有值，不会进 NO_FILL。本 skill 落地规则：
...
> `classRank`（成绩排名）已从配置删除，教育经历不再含此字段，不参与任何诊断/评分逻辑。
...
|---|---|---|
| `noInternshipExperience` | 16 | — |
| `havingInternshipExperience` | 16 | — |
| `havingWorkExperience` | 16 | — |
| `onlineApplication` | 18 | — |

> `familyMember` / `language` / `certificate` / `softwareOperate` / `paper` / `creation` 已从配置删除。专业能力三项命中：非网申类型只可能来自 `professionalSkillsNonOnline` 的 `languageSkill` / `qualificationCertificate` / `softwareOperation`；网申类型来自 `professionalSkills`。

## 8. 模块 sort（报告顺序依据）

- 解析路径：显示模块（按 `moduleConfig` JSON **数组顺序**）在前，隐藏模块（同序）在后，统一重新编号 sort=1..N。**不使用配置中的 `sort` 字段**。
- （对比：默认简历路径用的是配置 `sort` 字段排序。两条路径不同；诊断报告跟随解析路径。）
- 报告装配按模块 sort 升序遍历 → 报告里**显示模块按配置数组序排前，隐藏模块排后**（隐藏模块若有诊断明细也会输出，见 diagnosis-categories.md 的 NO_DATA 空流 quirk）。
...
1. **逐字段填值（强制）**：模板已列出**每个模块 × dataFieldList 每个字段**（含 showOnPage=false 字段），逐格给值，空就留 `""`；禁止跳过字段、禁止删字段。字段空白时由脚本正确记 NO_FILL。
...
4. **高频漏解析字段**（逐一对照原文确认有/无值）：
   - 基本信息：`workCity`（期望城市）、`position`（应聘岗位）、`idPhoto`（证件照，见 §2.2）；
...
## 附录 A：回归案例（2026-08-15，havingInternshipExperience）

同一份简历，线上基准输出 vs 修复前 skill 的差异及根因，作为回归基准：

| 现象 | 线上基准 | 修复前 skill | 根因 |
|---|---|---|---|
