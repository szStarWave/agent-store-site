...
description: 通过 WorkBuddy 中的 Kling AI 将自然语言需求优化为精确动态提示词并生成影视级、专业级视频。支持文生视频、图生视频、主体库 Element 复用及通过动作库或参考视频进行动作控制，适用于产品展示、广告短片和社交媒体内容。
---
...
使用主体库、动作库或需上传本地素材时，先阅读[素材工作流](../kling-ai-plugin/references/asset-workflows.md)。Element 先检查类型并绑定；库动作先取得真实 `motionId`；只查看素材库时交回核心 Skill，不提交视频。
...
- 优先使用宿主已提供且所选模型 schema 接受的图片引用，不要把本地路径直接传入 `inputs[]`。宿主未提供合规引用时，先按素材工作流检查两步上传；仍不可用则说明当前限制并停止。
