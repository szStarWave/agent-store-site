...
## queryMyCompletedTasks — 查询我的已办

```bash
beisen-cli approval task queryMyCompletedTasks [--data '{...}']
```
...
```json
{
  ...
  "data": {
    ...
    "data": [
      {
        "approvalObjectId": "f1514211-b6a8-449c-82e1-caf3e28d7236",
        "taskId": "f1514211-b6a8-449c-82e1-caf3e28d7236",
        "processTitle": "审批自由流",
        "approvalCategory": 55,
        "approvalCategoryName": "测试字段类型修改",
        "bizAppName": "BeisenCloudDemo",
        "processStatus": 1,
        "processStatusName": "审批中",
        "currentNode": "单人审批",
        "currentApproverId": [461183219],
        "currentApproverName": "BB",
        "startTime": "2025-05-27T14:58:25.167",
        "arrivalTime": "2025-05-27 14:58",
...
| `processStatusName` | string | 流程状态名称（如"审批中"、"已通过"、"已驳回"） |
| `currentNode` | string | 当前审批节点名称 |
| `currentApproverId` | number[] | 当前审批人ID列表 |
| `currentApproverName` | string | 当前审批人姓名 |
| `startTime` | string | 流程发起时间（ISO 8601 格式，如 `2025-05-27T14:58:25.167`） |
| `arrivalTime` | string | 任务到达时间（格式 `YYYY-MM-DD HH:mm`，如 `2025-05-27 14:58`） |
...
- 已办列表中的审批状态通过 `processStatus` / `processStatusName` 判断，不同状态值对应不同流程阶段
