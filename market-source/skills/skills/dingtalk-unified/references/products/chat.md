...
--group 指定群聊 ID 发群消息；--user 指定用户 userId 发单聊；--open-dingtalk-id 指定用户 openDingTalkId 发单聊。三者只能选其一，不能同时指定。消息内容为位置参数（恰好 1 个），支持 Markdown。**--title 为必填参数**（v1.0.28+ 群聊也必须传 --title，之前版本仅单聊必须）。
...
Example:
  dws chat message send --group <openconversation_id> --title "通知" --text "hello"
  dws chat message send --user <userId> --title "问候" --text "请查收"
  dws chat message send --open-dingtalk-id <openDingTalkId> --title "问候" --text "请查收"
...
# 3. 以个人身份发送群消息（--title 必填）
dws chat message send --group <openconversation_id> --title "周报提醒" --text "请大家本周五前提交周报" --format json
...
- `chat message send` 的 text 是位置参数（恰好 1 个），非 flag；**--title 为必填参数**（v1.0.28+ 群聊也必须传 --title）；群聊用 `--group`，单聊用 `--user`（userId）或 `--open-dingtalk-id`（openDingTalkId），三者互斥；`--at-all`、`--at-users` 仅在 `--group` 群聊时生效；发送图片消息用 `--media-id`
