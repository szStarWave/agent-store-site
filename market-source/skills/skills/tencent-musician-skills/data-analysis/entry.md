...
> 本文档是 `tencent-musician-skills` Skill 数据分析能力的完整入口。
> 主 `SKILL.md` 已在路由表中把「数据分析类」场景指向本文档；本文档负责业务流程、songId 前置流程、chat-sync 调用规范、对外展示规范。
>
> **详细补充规范**：`data-analysis/module-a-data-analysis.md`（完整场景示例 / 档位判定细则 / 话术模板）
> **登录态**：`references/auth.md`（已自包含，无需手动处理）

你是腾讯音乐人平台的数据分析助手，负责基于自然语言数据分析能力帮助用户洞察其音乐作品表现。

本部分**不走**「算子发现 / 算子详情 / 算子调用」的通用路由流程，而是**固定直接调用**下面这个同步接口：

> 📌 **固定接口**：`POST https://y.tencentmusic.com/openapi/v1/musician/data/agent/chat-sync`

| 能力模块        | 说明                                               | 详细文档                               |
| --------------- | -------------------------------------------------- | -------------------------------------- |
| 🔐 登录         | Skill 自包含登录态                                  | `references/auth.md`                   |
| 📊 智能数据分析 | 自然语言数据分析（同步）                           | `data-analysis/module-a-data-analysis.md` |

脚本文件在 `scripts/` 目录下，使用 Python 实现，仅依赖 Python 3 标准库，可在 macOS / Windows / Linux 上直接运行。

| 脚本                      | 作用                                                                                      |
| ------------------------- | ----------------------------------------------------------------------------------------- |
| `scripts/check_login.py`  | 登录态获取：优先读本地缓存，必要时自动触发扫码登录                                         |
| `scripts/verify_token.py` | 验证指定 Token 有效性                                                                       |
...
登录态已由本 Skill 自包含托管，所有 Token 统一维护在 **`~/.tme-login/token.json`**。`chat_sync.py` 内置读取逻辑，**绝大多数场景直接调用业务脚本即可**，无需显式获取 Token。

### 🚨 Token 处理铁律（最高优先级，严禁违反）

1. **严禁**绕过 `scripts/check_login.py` / `scripts/login.py` 自行实现登录流程
2. **严禁**把 Token 字符串作为命令行参数传给 `scripts/chat_sync.py`（脚本会自己读 `token.json`）
3. **严禁**让 Agent 凭记忆 / 从历史对话敲出 Token（Token 字符改一个就失效）
4. **严禁**手动往 `~/.tme-login/token.json` 写文件
5. 若用户在对话中直接贴了 Token，必须通过 `python3 scripts/check_login.py --manual` 走手动粘贴兜底（而不是自己解析）

### 获取登录态的标准流程

#### Step 1：调用 `scripts/check_login.py`（推荐，零感知）
...
1. 秒级命中 `~/.tme-login/token.json`
2. 无头 Playwright 基于 `storage_state.json` 刷新
3. 自动调起 `scripts/login.py` 做有头扫码登录（弹出 Chromium）
4. 手动粘贴 Token 兜底（`--manual`）
...
#### Step 2：Token 过期或 API 返回 UNAUTHORIZED
...
登录成功后 Token 会自动写入 `~/.tme-login/token.json`，后续调用自动复用。
...
| 情况                           | 处理方式                                                              |
| ------------------------------ | --------------------------------------------------------------------- |
| `~/.tme-login/token.json` 缺失 | `scripts/check_login.py` 会自动走无头刷新 → 扫码登录的完整链路       |
| `token.json` 中的 Token 失效   | 同上                                                                 |
| API 返回 `UNAUTHORIZED`        | 重新执行 `python3 scripts/login.py` 让用户扫码，然后重试业务          |
| 用户提供了新 Token             | 交由 `python3 scripts/check_login.py --manual` 处理                  |
...
> ✅ 反之，若用户问的是账号整体 / 多首歌概览（「我最近的数据怎么样」「整体播放量如何」「听众画像」「粉丝分布」），**不属于**单曲意图，**直接不传 songId** 正常调用即可。
...
> 📖 档 B 的"候选确认话术"与档 C 的"匹配失败兜底话术"完整模板详见 `data-analysis/module-a-data-analysis.md` → 「songId 获取流程与兜底话术」章节。**必须**使用该文档中的固定话术，严禁暴露 `songId`、"映射"、"候选池"、"相似度" 等技术字眼。
...
本业务的**唯一能力**就是调用下面这个同步接口拿到 Markdown 分析结论，没有任何路由分支。
...
- **自动从 `~/.tme-login/token.json` 读取登录态**；读不到再回退到环境变量 `TME_HEADER_TOKEN`；都拿不到则报错并提示先运行 `scripts/check_login.py`
...
> 详细的业务规范、`finishReason` 处理规则、展示规范与场景示例，见 `data-analysis/module-a-data-analysis.md`。
