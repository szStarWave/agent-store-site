...
1. **Read the config.** Read
   ...
   (and `~/.codebuddy/plugins/config/legal-workflows/company-profile.md` one
...
2. **Show the customizable map.** List what's in the profile, grouped, with a
   ...
   - **Risk calibration** — what counts as high / medium / low risk on an
     inbound demand, formal evidence or information request, or new matter; escalation triggers
   ...
   - **House style** — brief style, declaration format, demand letter
     template, witness interview outline structure, evidence preservation notice template
   ...
   - **Workflow** — matter workspaces, portfolio log, OC status cadence,
     evidence preservation notice refresh cadence
...
4. **Make the change.** Show the current value, ask for the new value, explain
   ...
   - *Risk calibration tightening high-risk threshold:* "More inbound
     demands and formal evidence or information requests will route through `/matter-briefing` and
     `/oc-status`."
...
5. **For shared-profile changes** (company name, industry, jurisdictions,
   ...
   `~/.codebuddy/plugins/config/legal-workflows/company-profile.md` and note:
...
- **Flag internal inconsistency.** If the change would make the profile
  ...
  tension.
- **Flag guardrail degradation.** The settlement-communication rule / privilege gate on
