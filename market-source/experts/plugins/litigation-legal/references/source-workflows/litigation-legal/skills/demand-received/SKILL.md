...
2. Load `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/_log.yaml` for portfolio cross-check.
3. Load `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` → risk calibration, landscape, demand-letter practice.
...
6. Write `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/inbound/[slug]/triage.md`. Copy or link incoming to `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/inbound/[slug]/incoming.[ext]`.
...
- `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/_log.yaml` — scan for related matters (same counterparty, overlapping counterparties via entity relationships, or matter type + recent date)
- `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` → risk calibration (for merit assessment), landscape (is the sender a frequent adversary?), demand-letter practice (house tone and response defaults)
...
- **Settlement-communication framing** — research the settlement-communication protections applicable in the forum (settlement-communication rule in federal, the state equivalent otherwise). Note whether the demand is marked as a settlement communication, but remember: protection attaches from conduct and context, not merely from labeling. Capture both the label (if any) and a first-pass read of whether the substance is in fact a compromise discussion.
...
- **Leverage and pressure** — are they credibly prepared to sue? Do they have capacity? Are they a repeat-litigant adversary per `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md`?
...
- Tradeoff: settlement-communication posture required — research the applicable rule (settlement-communication rule or state equivalent) and structure the response so the substance, not just the label, qualifies as a compromise discussion. Must be careful not to waive claims.
...
- Tradeoff: silence can be used against us in some contexts (e.g., account stated); evidence preservation notice still required
...
Output: `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/inbound/[slug]/triage.md`.
...
- Standalone → leave in `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/inbound/`; no portfolio change.
