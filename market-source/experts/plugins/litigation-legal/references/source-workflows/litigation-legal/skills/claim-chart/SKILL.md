...
1. Load `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` → role, work-product header, decision posture, document storage.
...
Check `## Matter workspaces` in the practice-level practice-profile.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `/litigation-legal:matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` — especially the case theory, the pleading / complaint (for the elements actually alleged), the jurisdiction, any Markman order or stipulated constructions (patent mode), and the phase of the case. Write outputs to the matter folder at `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/<matter-slug>/claim-charts/`. Never read another matter's files unless `Cross-matter context` is `on`.

---

## Load context

- `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` → role, work-product header, decision posture, document storage, case-theory scaffolding
- Active matter's `matter.md` — claims, defenses, side, jurisdiction, phase, theory
- For civil mode: the complaint or counterclaim (for the actually-pleaded counts), any answer (for the actually-pleaded affirmative defenses), the relevant pattern jury instruction source, and the governing statute if statutory. Also the evidence corpus — witness interview transcripts, declarations, produced documents, expert reports.
...
If `practice-profile.md` has `[PLACEHOLDER]` markers, surface this bounce:
...
**Conflicts gate — unbypassable.** Before building a claim chart, check `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:
...
> 2. **Civil element chart** — elements of a cause of action (or affirmative defense) mapped against the evidence. For complaint plausibility checks, evidence exchange planning, MSJ prep, order-of-proof outlines.
...
- **Phase.** Pre-filing, pleadings, evidence exchange, MSJ, trial prep, post-trial. The chart is the same; the framing of the output changes.
...
1. **Find evidence.** Accused product: documentation, manuals, data sheets, source code, teardowns, witness interview testimony, expert reports. Prior art: column/line for US patents, paragraph for published apps, page/figure for NPL. For prior art, flag whether the reference qualifies (§102(a)(1), (a)(2), (b); AIA vs. pre-AIA cutoffs). If prior-art status isn't obvious, mark `prior-art-status: needs-evidence`.
...
5. **Flag open questions.** "This maps if [X]. Need [teardown / source code / witness interview / expert] to confirm."
...
| Breach of fiduciary duty | Restatement / common law — fiduciary duty, breach, damages | **DE:** the most-developed body of fiduciary-duty law (*Aronson v. Lewis*, *Cede & Co. v. Technicolor*, *In re Trados*) — default to the chosen jurisdiction formulation for any DE-entity matter regardless of forum. |
...
- **State per cell** — `supported` / `partial` / `disputed` / `gap` / `needs-evidence exchange`.
...
> - If pre-evidence exchange: these are your evidence exchange priorities — the witness interviews, document requests, and interrogatories that turn a gap into `supported` or confirm `none`.
...
- **Discovery.** For each `gap` or `needs-evidence exchange` element, what evidence exchange is needed? Which witnesses, which document custodians, which interrogatories, which RFAs.
...
Prepend the work-product header from `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` `## Outputs`.
...
| 4 | Causation | — | — | none | needs-evidence exchange | ☐ |
...
- **Citation verification** — every pin cite, case, column/line, witness interview page:line must be verified against the source.
...
- Color coding by state:
  ...
  - *Civil:* white = `supported`, yellow = `partial` / `disputed`, orange = `needs-evidence exchange`, red = `gap`
...
- `_gaps` sheet listing every `gap`, `needs-evidence`, or `needs-evidence exchange` row with what's still needed
...
If matter workspaces enabled and a matter is active: `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/<matter-slug>/claim-charts/`. Otherwise: `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/claim-charts/`. Surface the path. Append a one-line entry to the matter's `history.md`.
...
>
> [Generate: claim / patent, side, jurisdiction, phase, elements, supported / gap / needs-evidence exchange counts, the three most load-bearing open questions.]

Deliver the chart alongside the brief.

## Shared guardrails — checklist

- **Citation verification.** Every pin cite (column/line, page, witness interview page:line, Bates, ¶) is a claim about the source. The attorney verifies. The skill does not fabricate cites — if a cite cannot be produced, the cell is `needs-evidence` or `gap`.
