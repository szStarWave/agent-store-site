...
description: House cold-start for the litigation plugin — branches by role (in-house, firm associate, solo) and side (plaintiff, defense, both), captures risk calibration, landscape, and house style, and writes the practice profile practice-profile.md. Use on a fresh install, when the user wants to set up or redo the practice profile, or to re-check available integrations.
argument-hint: "[--redo | --check-integrations]"
---

# /cold-start-interview

1. Check `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md`. If already populated and no `--redo`, ask before overwriting.
2. Follow the workflow and reference below.
3. Run Part 0 (role, side, integration check). The interview branches by role and side.
   - **Role** routes the practice profile structure: **in-house** (portfolio of matters, outside counsel oversight, reserve methodology, board/audit reporting), **firm associate** (case work — matter context, case theory and pivot fact, seed brief in house style, document review/priv-log setup), or **solo** (caseload + contingency or retainer economics + client expectations + SOL tracking, then the case-theory and brief-style sections).
...
5. Migration: if a populated practice-profile.md (no `[PLACEHOLDER]` markers) exists at `~/.codebuddy/plugins/cache/legal-workflows/litigation-legal/*/practice-profile.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md`. Date the footer.
7. Confirm with the user before finalizing: "Here's what I captured — anything wrong?"

## Flags

- `--redo` — re-run the full interview and overwrite `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` without re-running the full interview. Use after setting up a new connector (DMS, document storage, Gmail, scheduled-tasks, CLM).
...
Read `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md`:
- **Does not exist** → start the interview.
- **Contains `<!-- SETUP PAUSED AT: -->`** → greet the user and offer to resume from that section.
- **Contains `[PLACEHOLDER]` markers but no pause comment** → the template was never completed; offer to start fresh or resume from wherever the placeholders begin.
- **Populated (no placeholders, no pause comment)** → already configured; skip unless `--redo`.

The template structure lives at `${PLUGIN_ROOT}/practice-profile.md` — use it as the section scaffold. Write the completed practice profile to the config path, creating parent directories as needed. If a practice-profile.md exists at the old cache path `~/.codebuddy/plugins/cache/legal-workflows/litigation-legal/*/practice-profile.md` but not here, copy it forward.

## Check for the shared company profile

Look for `~/.codebuddy/plugins/config/legal-workflows/company-profile.md`.
...
> **`litigation-legal` is for people who work litigation — managing a portfolio of matters in-house, drafting briefs and doing evidence exchange at a firm, or both as a solo practitioner.** Not your area? `/legal-builder-hub:related-skills-surfacer`.
>
...
Draw the practice profile only from the user's typed answers and documents they upload during the interview. Do not read `~/practice-profile.md` or pull practice facts from ambient context. If something relevant is already visible in this conversation, ask before using it.
...
**Verify user-stated legal facts as they come up in setup.** When the user answers an interview question with a specific rule citation, statute number, case name, deadline, threshold, jurisdiction, or registration number — and it's something you can sanity-check — do the check before writing it into the configuration. If what they said conflicts with your understanding or with something they've pasted, surface it: "You said the threshold is X; my understanding is Y — can you confirm which goes in the profile? `[premise flagged — verify]`" A wrong fact written into practice-profile.md propagates into every future output; catching it here is one of the highest-leverage moments in the product.
...
> 2. **I'll pause before steps that have legal consequences** — sending a demand, responding to a formal evidence or information request, issuing or releasing a evidence preservation notice, filing a brief, submitting a confidentiality review log, designating documents in evidence exchange, closing a matter, accepting a settlement. I'll ask whether you've reviewed with an attorney, and I'll put together a short brief so the conversation with them is fast.
>
...
>
> **(b) At a firm doing brief drafting, evidence exchange, witness interview prep, document review** — you're the associate or paralegal responsible for actually producing the work product. One or a few matters, deep on each.
>
...
Ask this right after the role question. It's load-bearing for risk-calibration framing, demand-letter posture, evidence exchange stance, and the way chronologies are built.
...
> This plugin can work with: DMS (iManage), document storage (Google Drive, SharePoint, Box), Gmail, scheduled-tasks, CLM (Ironclad), document review (document-review platform, Relativity, DISCO, Aurora), legal research (research connector, legal research connector, research connector), outside-counsel recommendations (TopCounsel). Let me check which connectors you have configured — features that need them will work, and features that don't will fall back gracefully instead of failing silently.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Box isn't connected. In AI coworking environment: Settings → Connectors → Add → Box → sign in. In CodeBuddy: add the Box MCP to your config or via `/mcp`. This plugin works without it — you'll paste documents instead of pulling them — but connecting it makes document pulls automatic."
...
Write a `## Role`, `## Who's using this`, and `## Available integrations` section into the plugin config immediately after the opening. Add `## Outputs` with the work-product header rule per the practice-profile.md template.
...
- Document storage (2–3 min) — where matter docs live (filesystem, Drive, SharePoint, Box, Gmail, CLM, DMS, document review), default matter folder pattern, how docs get shared with OC.
...
- **Statute of limitations tracking** — how do you track SOL cutoffs across the caseload? (Calendar, case-management software, a paper case docket or procedural record, memory — whatever's real.) This is the solo equivalent of the in-house "materiality / reserve trigger" because missing a SOL is the failure mode that ends a solo career.
...
- **Evidence preservation notice** — even for a solo, preservation matters when litigation is anticipated. Template, if any. *Seed doc:* hold notice if issued.
...
After Section S3, continue to the **Firm-associate path** below. Solo practitioners write briefs, build chronologies, and prep witness interviews like firm associates do — the case-theory and seed-brief work applies.
...
- Stage (pleadings, evidence exchange, summary judgment, trial prep)
- Key dates coming up
...
If not:
- document review platform (document-review platform, Relativity, DISCO, Aurora)
- Review protocol — coding categories, who makes priv calls
- Confidentiality review log format
...
Write the completed practice profile to the plugin config, using the template at `${PLUGIN_ROOT}/practice-profile.md` as the section scaffold. Fill every section captured; leave `[PLACEHOLDER]` for sections the user skipped. Date the footer.
...
> - **Draft a demand letter** — e.g., "Privilege / settlement-communication rule gate, .docx output, post-send checklist, matter-creation offer." Try: `/litigation-legal:demand-draft`
...
> "Your practice profile is at `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
...
**Connect a research tool.** Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In CodeBuddy: authorize when a skill prompts you.
...
- Read personal `~/practice-profile.md` or other ambient context without asking.
