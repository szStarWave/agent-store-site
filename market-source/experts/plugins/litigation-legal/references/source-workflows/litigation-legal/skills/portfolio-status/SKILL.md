...
1. Load `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` → risk calibration (defines how to read the `risk:` field).
2. Follow the workflow and reference below.
3. Parse `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/_log.yaml`. Filter closed matters by default (include with `--all`).
...
- `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/_log.yaml` — source of truth
- `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` — risk calibration (to interpret risk/materiality fields correctly)
...
[table: pleadings / evidence exchange / dispositive motions / trial prep / settlement / appeal]
...
6. **Hold gap:** `status in [threatened, active, evidence exchange, trial, appeal]` and `legal_hold.issued == false` — preservation duty attaches at reasonable anticipation, so `threatened` matters are in scope.
...
If the portfolio has more than ~10 matters, or any time the user asks: offer the dashboard (see practice-profile.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by risk tier, a timeline of upcoming deadlines, and a sortable matter ledger with status, conflicts check, and last-touched date.
