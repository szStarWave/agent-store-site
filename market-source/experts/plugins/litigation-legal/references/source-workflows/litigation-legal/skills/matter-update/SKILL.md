...
2. Confirm slug exists in `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/` and `_log.yaml`.
...
4. Append dated entry to `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/[slug]/history.md`.
...
- `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/_log.yaml` — find the row
- `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/[slug]/history.md` — append target
- `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/[slug]/matter.md` — reference (don't rewrite)
- `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` — risk calibration (if re-assessing risk)
...
- **Discovery** — production made/received, witness interviews taken, formal evidence or information request served
...
- `status:` — has the stage shifted (e.g., pleadings → fact evidence exchange)?
...
If the Strategy update is a **settlement acceptance** (the company is accepting a settlement offer, executing a settlement agreement, or authorizing acceptance in principle — not merely logging an offer made or received): Read `## Who's using this` in `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md`. If the Role is Non-lawyer:
...
>
> [Generate a 1-page summary: the matter, proposed settlement terms (dollar, structural, release scope, confidentiality, non-disparagement), exposure at stake, authority ladder status (see `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` settlement authority), what could go wrong, what to ask the attorney before accepting.]
>
...
| Regulatory / enforcement development | "Regulator action (formal evidence or information request, CID, enforcement notice) usually triggers disclosure analysis. Current: `[current]`. Change?" |
...
If materiality moves to `reserved` or `disclosed`, and the matter did not previously carry a reserve or disclosure, flag the event as requiring finance / audit-committee notification per `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` materiality thresholds.
...
### Append to `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/[slug]/history.md`
...
### Update `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/_log.yaml`
