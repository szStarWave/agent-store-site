...
Your configuration is stored at `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` and survives plugin updates.

## Commands

| Command | Does |
|---|---|
| `/litigation-legal:cold-start-interview` | Cold-start → writes house `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` |
...
| `/litigation-legal:demand-draft [slug]` | Draft the letter from intake — runs settlement-communication rule / privilege gate, outputs `.docx`, writes post-send checklist |
...
| **demand-draft** | settlement-communication rule / privilege gate, then drafts `.docx` with `[CITE:___]` placeholders; writes post-send checklist; offers matter creation |
...
```
litigation-legal/
├── practice-profile.md                          # HOUSE practice profile — risk, landscape, style
├── matters/
...
│       ├── draft-v1.docx
│       └── checklist.md
├── inbound/                           # incoming demands, formal evidence or information requests, regulator letters
│   └── [slug]/
│       ├── incoming.[ext]
│       ├── triage.md
...
The legal research connectors in this plugin aren't just data sources — they're the difference between a verified citation and a citation you have to check. A citation retrieved through **research connector** (U.S. court opinions, PACER case docket or procedural records, citation verification), **research connector** (state trial court dataset — case docket or procedural records, rulings, verdicts, judge and opposing counsel analytics), **document-review platform** (your document review projects), or **Aurora** (read-only Consilio eevidence exchange — every record cited to source) is tagged with its source and can be traced back. A citation from the model's knowledge or from web search is tagged `[verify]` or `[verify-pinpoint]` and should be checked against a primary source before anyone relies on it. The plugin tiers its citations so your verification time goes where it matters.
...
Your practice profile at `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` isn't static — it improves as you use the plugin. Skills tell you when an output used a default you should tune. You can re-run setup, edit the file directly, or tell a skill to record a new position.

## Notes

- Every skill reads from `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` first. If your risk appetite changes or you bring on new outside counsel, update it — don't paper over it in individual matters.
- `## Company profile` is the first section of `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` by convention. If you run other `-legal` plugins, you can copy it across rather than re-entering the same context.
