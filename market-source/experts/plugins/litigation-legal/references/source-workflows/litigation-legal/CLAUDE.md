...
User-specific configuration for this plugin lives at a version-independent path that survives plugin updates:
  ...
  ~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md
...
4. On first run after a plugin update, if a populated practice-profile.md exists at the old cache path
   (~/.codebuddy/plugins/cache/legal-workflows/litigation-legal/<version>/practice-profile.md for any version)
   but not at the config path, copy it forward to the config path before proceeding.
5. This file (the one you are reading) is the TEMPLATE. It ships with the plugin and shows the
   structure the config should have. It is replaced on every plugin update. Never write user data here.

**Shared company profile.** Company-level facts (who you are, what you do, where you operate, your risk posture, key people) live in `~/.codebuddy/plugins/config/legal-workflows/company-profile.md` — one level above this file, shared by all 12 plugins. Read it before this plugin's practice profile. If it doesn't exist, this plugin's setup will create it.
-->
...
**Org / legal entity:** [PLACEHOLDER — e.g., "Acme Corporation, a chosen jurisdiction corporation"] *(From company-profile.md — edit there to change across all plugins)*
...
*Downstream skills read this to pick defaults: in-house uses portfolio / reserve / board-memo vocabulary; firm-associate uses case / partner review / document review vocabulary; solo uses caseload / contingency or retainer / client-update vocabulary. Never mix frames.*
...
- **EU:** No general work-product protection. Legal professional privilege (LPP) protects communications with external counsel for the purpose of legal advice, but internal analyses, DPIAs, compliance assessments, and launch reviews are generally NOT shielded from supervisory authorities. Art. 58(1) applicable privacy law gives DPAs broad investigative powers. A DG COMP dawn raid can seize a "privileged" launch review.
...
> **⚠️ Reviewer note**
> - **Sources:** [Research connector: research connector ✓ verified | not connected — cites from training knowledge, verify before relying]
...
If everything is green (research tool connected, full read, no flags, currency checked), collapse to one line: `⚠️ Reviewer note: research connector verified · full read · no flags · ready for your eyes`. Don't pad with bullets that all say "no issues."
...
**Next steps decision tree.** After an analysis, review, triage, or assessment, close with a decision tree — a draft of the OPTIONS, not a draft of the DECISION. The lawyer picks; AI assistant fleshes out. Format:
...
**What's data-heavy:** OSS scan results, patent/trademark portfolio registers, diligence issue grids, renewal/cancel registers, gap trackers, closing checklists, leave registers, matter ledgers, entity compliance calendars, confidentiality review logs, findings tables from any review. What's not: a 3-item issue list, a memo, a redline, a client letter. Use judgment — the test is "would a reader struggle to see the shape of this in text."
...
> "You mentioned a 4-year statute of limitations for willful working-time and wage rule violations — my understanding is it's 3 years (2 for non-willful). Can you confirm which you meant? `[premise flagged — verify]`"
...
- `[research platform]` / `[research connector]` / `[research connector]` / `[legal research connector]` / `[IP registry]` / `[statute / regulator site]` / `[user provided]` — where a cite actually came from. Provenance, not confidence. Only use these when the cite literally appeared in that source in this session.
...
A reviewer-note shorthand like "research connector verified" is honest only when a research tool actually returned the cite — it describes what the tool did, not what the skill's output is. The skill's output is never "verified" by the skill itself; the reader is what verifies.
...
**Verification log.** When you or the user verifies a flagged item — confirms a cite against a primary source, checks a deadline against the local rule, verifies a threshold against the current statute — record it so the next person doesn't re-verify. Write a one-line entry to `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/verification-log.md`:
...
When the user asks a question in this plugin's practice area — not just when they invoke a skill — read the practice profile at `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/practice-profile.md` (and `~/.codebuddy/plugins/config/legal-workflows/company-profile.md`) first, and apply it. If it's populated, answer as the configured assistant:
...
1. **Provenance tags describe what happened, not what you'd like to claim.** Tag a citation with the MCP source (e.g., `[research connector]`) only when the citation literally appeared in that tool's result this session. Model knowledge that "feels" like a research connector result is `[model knowledge — verify]`.
...
- **Say when you should be a team.** "This is a 500-document data room. A first-pass review at this scale is a document-review platform job (document-review platform, Relativity), not a single-agent task. I'll triage the first [N] and flag the rest for a platform run."
...
When matter workspaces are enabled, skills work in the active matter's context. Skills read this practice-level practice-profile.md for practice profile-level rules (risk calibration, landscape, house style) and the matter's `matter.md` for matter-specific facts and overrides. Outputs are written to the matter folder at `~/.codebuddy/plugins/config/legal-workflows/litigation-legal/matters/<matter-slug>/`.

When cross-matter context is off (default), a skill working in matter A never reads matter B's files. Learnings that should carry across matters are written to this practice-level practice-profile.md, not to a matter folder.
...
**Frequent fora:** [PLACEHOLDER — e.g., chosen jurisdiction Chancery, N.D. Cal., S.D.N.Y., AAA / JAMS arbitration]

### Document storage

*Where matter documents live. Skills like `chronology` read from these sources. In-house counsel often don't have a single document review platform; they have a patchwork. Name the patchwork.*
...
| [PLACEHOLDER e.g. "document-review platform"] | document review | — | [yes/no] |
...
**Matter documents shared with outside counsel via:** [PLACEHOLDER — e.g., "secure share link", "FTP", "their document review platform"]
