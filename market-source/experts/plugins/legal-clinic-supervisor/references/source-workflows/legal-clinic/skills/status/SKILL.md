...
1. Load `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` → supervision style, plain-language standards, jurisdiction.
...
`~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` → supervision style, plain-language standards (for client-facing), jurisdiction.
...
**Reader:** The client. Probably stressed. Possibly unfamiliar with legal process. Reading level per `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` plain-language standards (default 6th grade).
...
**Before sending:** sending a client status update is a consequential action. The gate is the supervision workflow in `## Supervision style` in `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md`, reinforced by the Part 0 role check confirming a licensed supervising attorney owns the setup. Confirm the draft has been reviewed per the supervision protocol (queue / flag / lighter-touch) and all internal review labels (`[AI-ASSISTED DRAFT]`, `[VERIFY]`, etc.) have been removed from the client-facing copy.
...
evidence exchange / motion pending / etc.]
...
- Current status of evidence exchange/motions/settlement
- What's outstanding
...
2. Current status: [evidence exchange status / motion status / settlement status]
...
Per `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md`:
