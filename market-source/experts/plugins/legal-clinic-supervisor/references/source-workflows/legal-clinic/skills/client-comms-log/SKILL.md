...
3. Route by flag:
   - `--add` (default): capture direction, medium, student, summary, action items, follow-up due. Confirm with user. Append (prepend most-recent-first) to `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/client-comms/[case-id]/log.md`.
...
- `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/client-comms/[case-id]/log.md` (if exists) — append target
- `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` → not heavily read; this skill is case-scoped
...
**Append** to `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/client-comms/[case-id]/log.md`. If the log doesn't exist, create it with a header:
