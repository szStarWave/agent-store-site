...
1. Check `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` is set up. If placeholders: "Ask [professor] to run `/legal-clinic:cold-start-interview` first."
...
`~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` → clinic profile, practice areas, jurisdiction, handbook path, supervision style, practice-area templates.
...
Read from `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` and the ingested handbook. Cover, interactively:
...
- **Supervision** — how review works in this clinic (per the supervision style in practice-profile.md). Be specific: "Before anything goes to a client or a court, [it goes in the review queue / you check with Professor X / etc.]"
...
- **[Supervision reminder per practice-profile.md style]** — what gets reviewed before it goes out, and how.
...
- What AI assistant can help with / what it can't (starting points yes, final work product no, authoritative citations no)
- Verification habits (the bullets from Part 4)
- Who to ask when stuck (professor name from practice-profile.md)
