...
description: >
  ...
  handbook/rules upload. Writes practice-profile.md so every other skill and every
  student who runs /ramp reads from the same clinic context. Use on fresh
  install, when practice-profile.md has placeholders, when re-doing setup with --redo,
...
1. Check `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md`. If populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated practice-profile.md (no `[PLACEHOLDER]` markers) exists at `~/.codebuddy/plugins/cache/legal-workflows/legal-clinic/*/practice-profile.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` including `## Who's using this` and `## Available integrations`. Show supervision choice and practice-area templates for confirmation.
...
Read `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md`:
...
- **Pause and resume.** Tell the supervising attorney up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/legal-clinic:cold-start-interview` again later and I'll pick up where you left off." When the attorney pauses, write a partial configuration to `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` with a `<!-- SETUP PAUSED AT: [section name] — run /legal-clinic:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the attorney: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
1. **Account tier and data-handling terms.** Your AI assistant account tier and its data retention and training policies — Team, Enterprise, Work, Education, and individual accounts have different guarantees about retention, use for training, and subprocessor handling. Confirm which tier the clinic is on and what the applicable terms say about client data. Document the answer in the plugin config.

2. **Client consent and disclosure practices for AI-assisted work.** Review clinical supervision body Formal Opinion 512 (2024), your state bar's AI guidance (if any), and Model Rules of Professional Conduct 1.1 (competence), 1.4 (communication), 1.6 (confidentiality), and 5.3 (supervision of nonlawyer assistance). Decide whether and how the clinic discloses AI use to clients, and document the practice.
...
4. **Practice-area heightened-confidentiality considerations.** Immigration, criminal defense, domestic violence, family, and some civil rights matters carry heightened confidentiality and security expectations that go beyond the baseline — adversary exposure risk, formal evidence or information request risk, safety risk for survivors. Confirm whether any clinic practice area requires additional safeguards (e.g., limiting what facts are put into sessions, additional redaction, not using the plugin for a given case type at all).
...
Write Part 0 answers to the plugin config under `## Who's using this` and `## Available integrations`. If a populated practice-profile.md exists at the old cache path `~/.codebuddy/plugins/cache/legal-workflows/legal-clinic/*/practice-profile.md` but not here, copy it forward first.
...
> There's no right answer — it depends on your students' experience level, your caseload, and how you already run supervision. You can change this later by editing practice-profile.md.
...
Per the practice-profile.md template. Key sections:
...
- **Ethical reminders calibrated to task:** e.g., /draft outputs remind about clinical supervision guidance supervision requirements
...
> - **Track deadlines across the active case docket or procedural record** — e.g., "See what's due in the next 14 / 7 / 3 / 1 days with warnings per your cadence." Try: `/legal-clinic:deadlines`
...
6. **Before your first case review, connect a research tool.** Say: "Before your first case review or memo: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In CodeBuddy: authorize when a skill prompts you."
...
> Done. Your clinic's configuration is at `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
