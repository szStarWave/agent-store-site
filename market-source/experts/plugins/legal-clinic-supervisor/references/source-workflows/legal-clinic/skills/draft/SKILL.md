...
1. Load `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` → practice-area templates, jurisdiction, local rules, supervision style.
...
`~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` → practice areas, practice-area templates, jurisdiction (state + local court + any local rules ingested), supervision style.

Case notes or intake summary for the facts.

## Pedagogy check

Read the supervisor guide for this practice area at `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/guides/<practice-area>.md`. Check the `pedagogy_posture` setting:
...
**Jurisdiction assumption.** The draft assumes the state, court, and local rules set in practice-profile.md. Caption format, service requirements, page limits, filing windows, and substantive rules vary materially across jurisdictions and even between courts in the same state. If the matter is in a different court or a different state, confirm with your supervisor before relying on any format, deadline, or argument in the draft.

## Workflow

### Step 1: Which document?

Match the request to the clinic's template set (from `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md`). Common set by practice area:
...
| **Housing** | Eviction answer, demand letter (repairs/witness interviewsit), motion to stay execution, evidence exchange requests |
...
Per `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` jurisdiction:
...
Filing a document with a court or agency is a consequential action. The gate is the supervision workflow in `## Supervision style` in `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md`, reinforced by the Part 0 role check that confirms a licensed supervising attorney owns the clinic setup. Court filings always route through supervision before filing, regardless of the supervision-style choice.
...
- [ ] [Supervision step per practice-profile.md style]
...
*clinical supervision body Formal Opinion 512 (2024): generative AI use requires competence,
