...
1. Load `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` → role (must be Supervising attorney), practice areas, jurisdiction.
...
5. Write `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/guides/<practice-area>.md`. Create the `guides/` directory if needed.
...
This is a supervisor skill. Read `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` → `## Who's using this` → Role. If the role is not "Supervising attorney," say:
...
Check the practice areas listed in `practice-profile.md` → `## Clinic profile` → Practice areas. If the chosen practice area is not listed there, note it: "I'll write this guide, but your practice profile doesn't list [area] as one of your clinic's practice areas. That's fine — you can add it later with `/legal-clinic:cold-start-interview --redo` — but the student-facing skills won't route intakes to this area until the profile lists it."

If a guide already exists at `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/guides/<practice-area>.md`, offer: "A guide for [area] already exists at [path]. Do you want to (a) revise it section-by-section, (b) start fresh and overwrite, or (c) see what's there first?"
...
**Last updated:** [date]
**Authored by:** [supervising attorney name from practice-profile.md]
...
**Court(s):** [from practice-profile.md or additional courts for this practice area]
...
The skill's "output" is the file written at `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/guides/<practice-area>.md`. The conversation with the supervisor is the interview; the written guide is the artifact.
