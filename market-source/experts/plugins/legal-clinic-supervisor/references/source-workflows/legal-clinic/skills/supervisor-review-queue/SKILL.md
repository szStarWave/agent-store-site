...
1. Check `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` → supervision style. If NOT "formal review queue": explain the clinic is set up for [flags/lighter-touch], no formal queue exists, and how to switch.
...
**This skill is only active if `~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` → Supervision style is "formal review queue."** Otherwise it's dormant — the cold-start interview asks the professor which model they want, and this is one of three options.
...
`~/.codebuddy/plugins/config/legal-workflows/legal-clinic/practice-profile.md` → supervision style. If NOT "formal review queue": respond with "The clinic is set up for [flags/lighter-touch] supervision — there's no formal queue. [Professor] reviews through [the clinic's existing structure]. To switch to a formal queue, edit practice-profile.md → Supervision style."
