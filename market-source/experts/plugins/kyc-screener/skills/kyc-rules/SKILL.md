...
Inputs: the structured record from `kyc-doc-parse`, the firm's rules grid (provided as a file by the user), and screening results (sanctions / PEP / adverse media) obtained via `WebSearch` against public lists.
...
| Sanctions / adverse media | public-list `WebSearch` result | Any hit → escalate |
