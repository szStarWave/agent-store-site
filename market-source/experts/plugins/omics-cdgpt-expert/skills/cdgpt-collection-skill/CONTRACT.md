...
## 1. 唯一合法出口：10 命令白名单 + 单合集锁定

cdgpt-collection-skill 通过 wrapper（`scripts/omics_cli.py`）调用 CLI；wrapper 在 argparse 层物理上注册一级命令，
本 SKILL **只允许**调用以下子集：
...
| 4   | `omics list public-apps`          | `--parent-app <CollectionAppId>`（合集展开） | read           | ✅ **仅限合集展开；必须带 `--parent-app 5e3b30ee-85a2-4ad7-80c5-fa2f0ecb0d0c`；禁止其他形态** |
...
| 10  | `omics quota`                     | `[-o json]`                            | read           | ✅ **C端专用**：配额查询（Step 0.3 首检 / Step 5.5 运行前检查，见 SKILL.md）                       |
