...
生成的产物 SKILL 只能调用以下命令，禁止越界：
    ...
    quota（C端体验用户配额查询）/ cos upload（本地文件上传 COS）
...
class OmicsCLI:
    ...
    def build_run(
        ...
        """
        COLLECTION_APP_ID = '5e3b30ee-85a2-4ad7-80c5-fa2f0ecb0d0c'  # 合集 AppId（硬编码，禁止覆盖）
        SUB_APPS = [{"AppId": "0f8efde4-1a1c-4b67-861c-1f03ec15261e", "AppName": "CD-GPT_reverse_translation_generation", "AppType": "NEXTFLOW", "AppDesc": "CD-GPT逆向翻译生成", "NextflowVersions": ["v24.04.3"]}, {"AppId": "995632d2-a434-458d-b2dc-19e1759c379b", "AppName": "CD-GPT_translation_generation", "AppType": "NEXTFLOW", "AppDesc": "CD-GPT正向翻译生成", "NextflowVersions": ["v24.04.3"]}]  # 子应用清单（生成时注入）
    ...
    def build_quota(self, output: str = "table") -> list[str]:
        """
        omics quota：查询 C端体验用户配额（RemainDays / TodayRemainingRuns）。
        SKILL 调用时机：Step 0.3 配额首检 & Step 5.5 运行前配额检查（仅 C端用户，强制执行）。
        """
        return [self.cli_path, "quota", "-o", output]

    # --- 13. cos upload（本地文件 → COS，见 SKILL.md §COS 上传流程）---
    def build_cos_upload(
        self,
        file_path: str,
        cos_path: str,
        output: str = "table",
        ...
        """
        omics cos upload：将本地生成的文件上传至用户绑定的 COS 桶，返回 COS URL。
        ⚠️ SKILL 守则：必须先向用户确认 COS 目标路径后才可执行上传（先确认再上传）。
        """
        if not file_path:
            raise ValueError("cos upload: file_path 不能为空")
        if not cos_path:
            raise ValueError("cos upload: cos_path 不能为空")
        return [self.cli_path, "cos", "upload", "-o", output,
                "--file", file_path, "--cos-path", cos_path]
...
def main():
    ...
    # 8. quota（C端体验用户配额查询）
    qt = subparsers.add_parser("quota", help="查询 C端体验用户配额（RemainDays / TodayRemainingRuns）")
    qt.add_argument("-o", "--output", default="table", choices=["table", "json"])

    # 9. cos（COS 对象存储操作）
    cos_p = subparsers.add_parser("cos", help="COS 对象存储操作（upload）")
    cos_sub = cos_p.add_subparsers(dest="cos_action")
    cos_up = cos_sub.add_parser("upload", help="上传本地文件到用户绑定的 COS 桶")
    cos_up.add_argument("--file", dest="file_path", required=True,
                        help="本地文件绝对路径或相对路径")
    cos_up.add_argument("--cos-path", dest="cos_path", required=True,
                        help="COS 目标路径（相对于用户配置的 CosBucketName）")
    cos_up.add_argument("-o", "--output", default="table", choices=["table", "json"])
    ...
    try:
        ...
        if args.command == "login":
            ...
            if args.cos_action == "upload":
                cmd_args = cli.build_cos_upload(
                    file_path=args.file_path,
                    cos_path=args.cos_path,
                    output=args.output,
                )
            else:
                cos_p.print_help(); sys.exit(1)
        else:
