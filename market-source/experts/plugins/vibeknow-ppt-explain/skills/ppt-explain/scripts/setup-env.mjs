...
// ① 渲染依赖(含 pptx 渲染库 @aiden0z/pptx-renderer + puppeteer-core,PPTX 转图要用)
// node_modules 不存在,或存在但缺关键依赖(如新加的 pptx 渲染库)→ 都跑一次 npm install(幂等)。
const needInstall = !existsSync(path.join(RENDER, "node_modules")) ||
                    !existsSync(path.join(RENDER, "node_modules", "@aiden0z", "pptx-renderer"));
if (needInstall) {
  log("装渲染依赖(remotion + react + pptx 渲染库)…");
