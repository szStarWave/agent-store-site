...
def _get_config(token: Optional[str] = None, host: Optional[str] = None) -> tuple[str, list[str]]:
    """Return (token, hosts_to_try) using Taihu auth and the Databrain MCP host."""
    _load_dotenv()
    token = (token or os.environ.get("TAI_IT_TOKEN", "")).strip()
    if not token:
        raise RuntimeError(
            "TAI_IT_TOKEN not set. "
            "请前往 https://tai.it.woa.com/user/pat 获取\"授权访问应用-全部应用\"的 token，"
            "并设置环境变量 TAI_IT_TOKEN（不含 Bearer 前缀）。"
        )
