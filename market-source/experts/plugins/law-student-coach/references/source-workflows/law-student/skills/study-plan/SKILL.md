...
name: study-plan
description: >
  Build or update a long-term exam prep (or exam prep) study plan — phases,
  ...
  "plan my exam prep", "schedule my studying", or "how should I study for [X]".
argument-hint: "[--build | --update | --status | --cram]"
---

# /study-plan

1. Load `~/.codebuddy/plugins/config/legal-workflows/law-student/practice-profile.md` → bar jurisdiction, exam format, bar date, weak subjects, target study hours/day, prep course.
2. Load `~/.codebuddy/plugins/config/legal-workflows/law-student/study-plan.yaml` if it exists.
...
`~/.codebuddy/plugins/config/legal-workflows/law-student/study-plan.yaml` if it exists — extend, don't overwrite.
...
Write to `~/.codebuddy/plugins/config/legal-workflows/law-student/study-plan.yaml`:
