...
2. Load `~/.codebuddy/plugins/config/legal-workflows/law-student/practice-profile.md` → jurisdiction, exam format, weak subjects.
3. Load `~/.codebuddy/plugins/config/legal-workflows/law-student/study-plan.yaml` if it exists. Read `session_history` for this subject to weight subtopics toward where the student has been weak.
...
6. At session end, write session results:
   ...
   - If not: write to `~/.codebuddy/plugins/config/legal-workflows/law-student/session-history.yaml`.
7. Report:
