...
function usage() {
  ...
  chatcut-upload-media --token <token> --endpoint <url> <file> [file...] [--background]
  chatcut-upload-media --token <token> --endpoint <url> --input <file> [--input <file>...] [--json-out <result.json>] [--background]
  chatcut-upload-media --token <token> --endpoint <url> --background <file> [file...]
  chatcut-upload-media --token <token> --endpoint <url> --asset-id <assetId> <file>
  chatcut-upload-media --token <token> --endpoint <url> --transcription-only --asset-id <assetId> <file>
...
Run only through the WorkBuddy package command chatcut-upload-media. Node.js 18+ and ffmpeg/ffprobe on PATH are required.
`);
}
...
function assertTool(bin, label) {
  ...
  if (result.error || result.status !== 0) {
    fail(
      `${label} is required for ChatCut media import. Install FFmpeg and ensure ffmpeg and ffprobe are on PATH. macOS: https://www.osxexperts.net/ Windows: https://www.gyan.dev/ffmpeg/builds/`,
    );
  }
}

function resolveMediaTool(configured, label) {
  const bin = configured || label;
  assertTool(bin, label);
  return bin;
}

function resolveMediaTools(options) {
