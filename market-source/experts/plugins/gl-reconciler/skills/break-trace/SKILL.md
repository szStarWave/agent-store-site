...
1. **Pull the GL side** — from the GL extract the user attached (Excel / CSV / system export), locate the journal entry or posting that produced this GL line: entry id, posting date, source system, batch id, preparer.
2. **Pull the subledger side** — from the subledger extract the user attached, locate the matching transaction: trade id, trade/settle dates, counterparty, source feed, FX rate used.
