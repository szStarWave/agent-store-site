...
1. **FIRST: Public filings.** Pull primary financials from the issuer's own filings — SEC EDGAR (10-K / 10-Q / 8-K), 巨潮资讯 (A-share annual / interim / quarterly reports), HKEX disclosure, or the company's IR site. Numbers in audited filings are the gold standard.
2. **SECOND: Reputable third-party reports** with clear attribution — sell-side research, exchange data pages, regulator publications. Always record the source name and access date.
3. **`WebSearch` + `WebFetch`** are the working tools. Use them to locate filings and reports; do not paraphrase numbers from blog posts or unverified summaries.
4. **NEVER** invent or estimate a multiple, growth rate, or margin without a citation. Mark it `[UNSOURCED]` instead.

**Why this matters:** Comps analysis is only as credible as its sources. Every number in the spreadsheet must trace back to a public, verifiable document an analyst can re-pull.

---

## Overview
This skill helps the assistant build institutional-grade comparable company analyses that combine operating metrics, valuation multiples, and statistical benchmarking. The output is a structured Excel/spreadsheet that enables informed investment decisions through peer comparison.
...
- Where did the data come from? (e.g. SEC EDGAR 10-K, 巨潮资讯 annual report, HKEX filing, exchange data page, sell-side research with name and date)
- What period does it cover? (Q4 2024, audited figures)
- How was it verified? (Cross-checked against the issuer's own filing)
- Note: Always prioritize the issuer's own audited filings as the primary source; treat secondary or aggregated sources as cross-checks, not substitutes.
...
2. **Add cell comments to ALL hard-coded inputs** - Right-click cell → Insert Comment → Document source OR assumption
   ...
   **For sourced data, cite exactly where it came from:**
   - Example: "MSFT Q4 FY2024 10-K, page 65, 'Total Revenue', accessed 2024-10-02"
   - Example: "Q4 2024 10-K filing, page 42, line item 'Total Revenue'"
   - Example: "Sell-side consensus EPS, mean of 12 analyst estimates as of 2024-10-02"
...
2. **Gather data** (60-90 minutes)
   - Pull from primary sources: SEC EDGAR / 巨潮资讯 / HKEX disclosure / company IR sites; cross-check with reputable third-party reports
...
- **Link to source files**: Hyperlink to the underlying filing, exchange page, or research report
...
- [ ] **Hyperlinks added where relevant** (SEC EDGAR filings, 巨潮资讯 announcements, exchange data pages, research reports)
