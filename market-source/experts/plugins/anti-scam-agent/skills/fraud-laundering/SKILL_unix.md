...
# fraud-laundering能力说明（macOS / Linux）
...
```shell
AUTH_CONFIG='{"Sign":"DQsZXjYVSigaJjNXVQAPQRIEARQ2GwgQHRVDYQ9fIEEGFVc2PRw6XSw9BmksTw0LKnhTEkpCHElYLh1ENkdZBDg8WhZMK1VKEgMmEzhMSxQ6fQZ/IydYKww7ez0xMxY8FAYOQiY7WTRdGF5VGGg7VTlcOBo9R11YGiJRPEEpEQ==","CredentialId":"crd-HvjD1b3q","ResourceID":"mcp-280b0453","InstanceID":"ins-3b7b6eb8"}' \
MCP_CONFIG='{"McpName":"fraud-laundering","McpURL":"http://lb-hcl91l4w-9x3l6p5id8zfs7h1.clb.nj-tencentclb.cloud/_llmsgw_/mcp/aga-6588cc10/mcp-280b0453"}' \
...
- 羊毛党、料商、四件套、泛黑灰产生态链用 `dark-grey-intel`。
...
| `black_card_transaction_stats` | 黑卡交易 | 用黑卡做的**具体交易笔数** |
...
- `dimensions`：分组维度：bank / province / city / date。（类型 `array`）⚠️ 必须按 JSON 字符串字面量传参：用单引号包裹整个 `key=value`，内部 JSON 双引号写成 `\"`，如 `'dimensions="[\"bank\"]"'`；**不要**写成裸数组 `dimensions=["bank"]`，否则维度会被吞，服务端收到 `dimensions:null`，只返回窗口总量单行。
...
amcpcli call black_card_transaction_stats 'dimensions="[\"bank\"]"' start_date=2026-06-01 end_date=2026-06-30
...
- `filters`：过滤条件；支持字段 date。（类型 `object`）
- `limit`：返回行数上限，默认 100。（类型 `number`）
- `order_by`：排序：value_desc / value_asc。（类型 `string`；可选：`value_desc` / `value_asc`）
- `start_date`：起始日期，YYYY-MM-DD。（类型 `string`）
...
- `limit`：返回条数上限（每个数据源），默认 500，最大 20000。注意：单次关键词 + 银行组合可能命中较多，定性样本建议 `limit=200~500`。（类型 `number`）

调用示例：

```bash
# 复杂关键词表达式：农业银行/农行 + 卡U
amcpcli call laundering_evidence_search keyword='"(农业银行 OR 农行) AND 卡U"' include_iocs=true source='all' start_date='2026-06-15' end_date='2026-07-01' limit=15
```
...
AUTH_CONFIG='{"Sign":"DQsZXjYVSigaJjNXVQAPQRIEARQ2GwgQHRVDYQ9fIEEGFVc2PRw6XSw9BmksTw0LKnhTEkpCHElYLh1ENkdZBDg8WhZMK1VKEgMmEzhMSxQ6fQZ/IydYKww7ez0xMxY8FAYOQiY7WTRdGF5VGGg7VTlcOBo9R11YGiJRPEEpEQ==","CredentialId":"crd-HvjD1b3q","ResourceID":"mcp-280b0453","InstanceID":"ins-3b7b6eb8"}' \
MCP_CONFIG='{"McpName":"fraud-laundering","McpURL":"http://lb-hcl91l4w-9x3l6p5id8zfs7h1.clb.nj-tencentclb.cloud/_llmsgw_/mcp/aga-6588cc10/mcp-280b0453"}' \
amcpcli call laundering_stats_query 'dimensions="[\"date\"]"' filters='{"laundering_method":"卡U"}' start_date='2026-06-01' end_date='2026-06-30' limit=100 order_by='value_asc'
```
