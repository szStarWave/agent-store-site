...
description: "tFold 抗体/纳米抗体与抗原复合体结构预测模型系列，含 ab_predict、ag_predict、ag_predict_icf、tcr_predict 四子应用。"
triggers: [tFold Collection (Nextflow), tfold collection (nextflow), tfold-collection-(nextflow), tfold_collection_(nextflow), tfoldcollection(nextflow), 蛋白质结构预测, 跑 tFold Collection (Nextflow), run tfold collection (nextflow)]
...
**应用简介**：tFold 抗体/纳米抗体与抗原复合体结构预测模型系列，含 ab_predict、ag_predict、ag_predict_icf、tcr_predict 四子应用。
...
| ---- | ---------- | ---- | --------------- | ----------- | ---- |
| 1 | `tFold_ab_predict` | `NEXTFLOW` | `31d80b72-6fea-44d3-9eb5-4725584199ef` | v24.04.3 | tFold抗体预测 |
| 2 | `tFold_ag_predict` | `NEXTFLOW` | `864d13a9-b162-432a-bfda-7f0b6bf137e7` | v24.04.3 | tFold抗原预测 |
| 3 | `tFold_ag_predict_icf` | `NEXTFLOW` | `2286b426-09eb-47d4-9608-325651d11157` | v24.04.3 | tFold抗原ICF预测 |
| 4 | `tFold_tcr_predict` | `NEXTFLOW` | `0ba6d880-27f1-475e-b4ec-d28885ab2b81` | v24.04.3 | tFold TCR预测 |
...
本 SKILL 仅服务于 `tFold Collection (Nextflow)` 合集（AppId: `807fc9ae-6197-43c0-a7a8-993c09ec1ee2`）。
...
> tFold Collection (Nextflow) 包含以下子应用，请选择一个：
>
  1. **tFold_ab_predict** (`NEXTFLOW` · NF版本: v24.04.3) — tFold抗体预测
  2. **tFold_ag_predict** (`NEXTFLOW` · NF版本: v24.04.3) — tFold抗原预测
  3. **tFold_ag_predict_icf** (`NEXTFLOW` · NF版本: v24.04.3) — tFold抗原ICF预测
  4. **tFold_tcr_predict** (`NEXTFLOW` · NF版本: v24.04.3) — tFold TCR预测
>
> 请输入编号或应用名称进行选择。
...
- `<selectedAppId>` → 来自上方子应用清单的 AppId，**不得使用合集本身的 AppId（807fc9ae-6197-43c0-a7a8-993c09ec1ee2）**
...
> - `<selectedAppId>` 来自本 SKILL 子应用清单，**不得使用合集本身的 AppId（807fc9ae-6197-43c0-a7a8-993c09ec1ee2）**
...
### 场景 B：用户直接说出子应用名
用户说「帮我跑 tFold Collection (Nextflow) 里的 XXX」→ Step 3 精确匹配 → 确认 → 同名检查 → 运行
...
**场景：使用 AI 生成的蛋白质序列运行 tFold Collection (Nextflow) 子应用**

```
用户: 生成测试蛋白质序列，使用默认模板运行tFold Collection (Nextflow)
...
Agent:
  ...
  Agent:
    ...
    6. [继续] omics run --public-app 807fc9ae-6197-43c0-a7a8-993c09ec1ee2 --input /tmp/run_input.json ...
```
...
# cli.execute(cli.build_run(public_app="807fc9ae-6197-43c0-a7a8-993c09ec1ee2", ...))      # 合集本身不可 run
