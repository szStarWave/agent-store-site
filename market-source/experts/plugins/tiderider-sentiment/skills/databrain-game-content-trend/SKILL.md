...
| 变量名 | 必填 | 默认值 | 说明 |
|--------|------|--------|------|
| `DATABRAIN_TOKEN` | 是 | — | DataBrain 认证 token 原始值（**不含** `Bearer ` 前缀），脚本会自动拼接；不要写死在代码中。 |
| `DATABRAIN_HOST` | 否 | `https://databrain.mcp.it.woa.com` | API 主机地址；本 skill 的内容趋势接口仅在该域名可用，显式设置后仅允许受信任域名（`databrain.mcp.it.woa.com`） |

Token 由环境变量或已安装 plugin 根目录 `.env` 自动读取。若为空，引导用户前往 DataBrain 个人令牌中心（内网 `https://databrain.woa.com/v2/user-center/personal-tokens-center`，外网 `https://databrain-global.intlgame.com/v2/user-center/personal-tokens-center`）获取"授权访问应用-全部应用"的 token，设置 `DATABRAIN_TOKEN` 即可，不要加 `Bearer` 前缀。
