#!/usr/bin/env python3
"""
publish_gallery_html.py — hotposts skill 专用 AI Gallery 发布脚本。

职责边界：
  1) 使用当前用户 token 上传热帖 HTML 到 AI Gallery；
  2) 保持作品 visibility 为 self；
  3) 回读并输出可访问 URL；
  4) 为接口可行性验证提供 --set-self-only 模式。

外层 skill / publish_digest.py 只需要调用本脚本并读取 stdout JSON，不需要感知 Gallery
底层 API 细节。
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Publish hotposts HTML to AI Gallery and keep visibility=self.")
