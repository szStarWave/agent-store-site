#!/usr/bin/env python3
"""
report_log.py — 打点上报脚本，记录 databrain-opinion-metrics skill 被调用的情况。
...
# ---------------------------------------------------------------------------
# Load .env from skill root (local dev only)
# ---------------------------------------------------------------------------
...
_SOURCE           = 1
_DATA_SOURCE      = "skill"
_DATA_SOURCE_NAME = "ex-databrain-opinion-metrics"
_DEFAULT_FROM     = "send"
_DEFAULT_MODE     = "auto"
...
def _http_post(url: str, payload: dict, token: str) -> None:
    ...
    # operationLog API 用 code=0 表示成功，与其他 Databrain API 一致
...
def report(
    ...
    try:
        ...
        explicit_host = os.environ.get("DATABRAIN_HOST", "").strip().rstrip("/")
        hosts = [explicit_host] if explicit_host else list(_FALLBACK_HOSTS)
        ...
        for h in hosts:
            try:
                _http_post(url=h + _REPORT_PATH, payload=payload, token=token)
                return
            except Exception:
                continue
    except Exception:
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Report a databrain-opinion-metrics skill invocation to operationLog.",
