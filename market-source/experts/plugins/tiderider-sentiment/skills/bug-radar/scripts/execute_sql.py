...
Environment variables:
  - DATABRAIN_TOKEN: DataBrain Token（raw JWT value, no `Bearer ` prefix; required）
  - DATABRAIN_HOST:  DataBrain API host (optional; default `https://databrain-global.intlgame.com`).
                     Only trusted hosts are accepted; non-trusted hosts fall back to default with a warning.

⚠️ TIMEZONE — UTC+8 (Beijing time):
  舆情数据全栈口径为 **UTC+8（北京时间）**。写任何含时间过滤的 SQL **之前**，
  必须先跑 `python scripts/now_beijing.py` 拿权威 `today`（UTC+8 北京时间日期，
  与 react_agent_service 进程时钟和 BigQuery 服务端 UTC 时钟均完全脱钩）。
  ...
  **禁止** `CURRENT_TIMESTAMP() / CURRENT_DATE() / CURRENT_DATETIME()` —— BQ
  ...
  See also: `scripts/now_beijing.py`（canonical UTC+8 today provider），
  以及 SKILL.md 顶部 Hard Constraints 时间过滤铁律 + Phase 2 时间映射表。
...
Usage:
    ...
    python execute_sql.py --sql_file /tmp/query.sql

Large result sets are automatically spilled to /tmp/opinion_sql_<timestamp>.json and stdout gets a summary + first
N rows sample. See --max_stdout_bytes / --output_file / --head_rows.
...
| `references column X which is neither grouped nor aggregated` | Missing GROUP BY column | Add to GROUP BY or wrap in aggregate; use ordinal positions |
| 61003 security check failed | Non-SELECT statement or forbidden keyword in SQL text | Strip DDL/DML; this skill's pre-check should catch most cases |
...
| `Unrecognized name: language` | Some Cube views expose derived `language_code`; raw BQ field is `language` | See references/cube_schema.md §3 |
| Empty result | Genuinely no data, OR a non-existent column silently filtered out | Probe with `SELECT <known_field> LIMIT 1` first |
...
def _spill_path(explicit: str | None) -> str:
    """Resolve the target spill path."""
    if explicit:
    ...
    auto = Path("/tmp") / f"opinion_sql_{ts}.json"
    try:
        auto.parent.mkdir(parents=True, exist_ok=True)
        if os.access(str(auto.parent), os.W_OK):
            return str(auto)
        raise OSError("/tmp not writable")
    except OSError:
        print("⚠ /tmp not writable, falling back to cwd for spill file", file=sys.stderr)
        return str(Path.cwd() / f"opinion_sql_{ts}.json")
...
def main():
    ...
    if should_spill:
        try:
            path = _spill_path(args.output_file)
            spill_bytes = _write_spill(result, path)
        except OSError as e:
            print(
