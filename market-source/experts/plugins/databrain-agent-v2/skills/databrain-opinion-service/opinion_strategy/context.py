...
class AgentContext(BaseModel):
    ...
    @field_validator("business_models", **_FIELD_VALIDATOR_KW)
    def _normalize_business_models(cls, value):
        ...
        for key, val in value.items():
            ...
            if isinstance(val, dict):
                for candidate in ("current", "status", "business_model", "value"):
                    candidate_val = val.get(candidate)
                    if isinstance(candidate_val, str) and candidate_val.strip():
                        normalized[key_str] = candidate_val
                        break
                else:
                    normalized[key_str] = json.dumps(val, ensure_ascii=False, default=str)
                continue
            if val is None:
                normalized[key_str] = ""
                continue
    ...
    dashboard_pc_enable_game_list_raw: Dict[str, dict] = Field(default_factory=dict)  # {game_code: {"game_name_en": str, "game_type": str}}
