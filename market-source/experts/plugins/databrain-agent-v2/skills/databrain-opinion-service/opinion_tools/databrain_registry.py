...
Import chain (circular-import safe):
    tools/registry.py  (no imports from model_tools or tool files)
           ^
    tools/*.py  (import from opinion_tools.registry at module level)
           ^
...
class ToolRegistry:
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from opinion_tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
...
#
# Usage:
#   from opinion_tools.registry import registry, tool_error, tool_result
#
