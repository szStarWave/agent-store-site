# Bayesian Statistical Analysis (Optional)

This document provides guidance on Bayesian analyses. **For most BI scenes (Benchmarking, A/B, Channel Effectiveness, etc.), frequentist tests (t-test, ANOVA, chi-square) are sufficient.** Use Bayesian methods when you need prior information, credible intervals, or evidence for the null. Load stat-t-test, stat-anova, stat-chi-square references for primary implementation.
...
**Python calculation**:
```python
# Note: Limited BF support in Python; use R (BayesFactor), JASP
