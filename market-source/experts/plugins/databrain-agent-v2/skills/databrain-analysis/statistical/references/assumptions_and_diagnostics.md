...
Guidance on checking assumptions. For t-test/ANOVA: use scipy (or stat-* references). **For regression diagnostics (VIF, Breusch-Pagan, Durbin-Watson)** → load **stat-diagnostics** reference.

BI note: **revenue, DAU, downloads, LTV** are often right-skewed — plan for Welch's or non-parametric.

## General Principles

1. **Always check assumptions before interpreting test results**
2. **Use multiple diagnostic methods** (visual + formal tests)
3. **Consider robustness**: Parametric tests are often robust with n > 30 per group
4. **Document checks and remedial actions** in reports
5. **BI metrics**: If skewed, use Mann-Whitney, Kruskal-Wallis, or Welch's t-test/ANOVA
...
**Formal tests** (secondary):
- Levene's test (t-test/ANOVA): scipy.stats.levene
- Breusch-Pagan, Durbin-Watson, VIF (regression): **load stat-diagnostics reference**
...
**Regression diagnostics** (Breusch-Pagan, VIF, residuals): Load **stat-diagnostics** reference.
...
**→ Load stat-diagnostics reference** for Durbin-Watson, Breusch-Pagan, VIF, Cook's distance, OLS diagnostics.
