...
Guidelines for reporting statistical analyses. For BI/game analytics, use the **BI Quick Report** format; for academic/publication, see the detailed sections below.

## BI Quick Report (Recommended for Data Agent)

For Benchmarking, A/B, Channel Effectiveness, Segmentation, etc., report:

1. **Descriptive stats**: Mean (or median if skewed), SD, n per group
2. **Test result**: Test name, statistic, df, exact p-value
3. **Effect size**: Cohen's d, η², or Cramér's V with 95% CI
4. **Assumption check**: Brief note (e.g., "Normality OK"; "Used Welch's due to unequal variance"; "Used Kruskal-Wallis (non-normal)")
5. **Conclusion**: One sentence (e.g., "Channel A retention significantly higher than B, d = 0.5, p = .02")

**Example (t-test):** "Revenue: Channel A M = 1250 (SD = 320, n = 50), Channel B M = 1080 (SD = 290, n = 48). Welch t(95.2) = 2.78, p = .006, d = 0.56 [0.15, 0.96]. Channel A significantly higher."

**Example (ANOVA):** "Retention by segment: F(2, 147) = 6.2, p = .003, η²_p = .08. Post-hoc: High-value > Low-value (p = .001), High = Mid (p = .21)."
...
2. **Completeness**: Include planned analyses and outcomes
3. **Honesty**: Report non-significant findings and assumption violations
4. **Clarity**: Write for your audience
5. **Effect sizes**: Always report with p-values

---

## Pre-Registration and Planning (Optional for BI)
