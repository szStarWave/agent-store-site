...
- Default to using all information the user provides plus `WebSearch` + `WebFetch` against public filings and reputable third-party sources for any inputs the user has not attached.
...
Fetch data from user-provided files and the web.

**Data Sources Priority:**
1. **User-Provided Data** — Historical financials and assumptions the user attaches
2. **Public filings** — SEC EDGAR (10-K / 10-Q), 巨潮资讯 (A-share annual / interim / quarterly reports), HKEX disclosure, company IR sites
3. **`WebSearch` + `WebFetch`** — Current prices, beta, net debt/cash, sell-side consensus when not in attached files
...
1. **Gather market data**:
   - Use `WebSearch` + `WebFetch` for stock prices, beta, and other market metrics from exchange pages and reputable financial data sites
   - Request from user if specific data is needed

2. **Gather historical financials**:
   - Pull from the issuer's own filings (SEC EDGAR / 巨潮资讯 / HKEX) via `WebSearch` + `WebFetch`
   - Use any historical extracts the user attaches as additional inputs
   - Manual extraction from 10-Ks / annual reports if necessary
