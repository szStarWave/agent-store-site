...
| 用途 | 路径 |
|---|---|
| 主日志 | `/data/services/pcmgr_enterprise/logs/acl-info/acl-info.log` |
| 错误 | `.../acl-info/error_acl-info.log` |

> ⚠️ **无 fatal**。

### 串联查询字段：tenant_id、device_id(mid)、account_id。
...
### 场景 A：采集/风险查询失败（影响 acl-strategy 判定）
> [安全化移除] 原始资料中的执行片段、敏感材料索取或变更步骤已删除；仅可按公共规范进行只读、限时、脱敏核实。
...
> acl-info 是 ACL 基础信息，表与 acl-strategy 共用。
> 访问控制策略真源：**acl_strategies**（见 acl-strategy 页）。