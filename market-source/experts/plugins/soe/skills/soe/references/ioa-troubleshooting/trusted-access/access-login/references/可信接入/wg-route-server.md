...
# wg-route-server（WireGuard 隧道服务端）· 排障知识库
...
| 用途 | 路径 |
|---|---|
| 主日志（单文件） | `/data/services/pcmgr_enterprise/public/service/network-mgr/wg-route-server/logs/wg_route_server.log` |

> ⚠️ **单文件、无独立 error/fatal**（network-mgr 系列均如此，日志在 public/service/ 层）。
