...
# flow-collect（网络流量采集）· 排障知识库

> 本文档按**服务粒度**独立成库，供 AI 助手消费。flow-collect = 网络流量采集。
...
| 用途 | 路径 |
|---|---|
| 主日志 | `/data/services/pcmgr_enterprise/logs/flow-collect/flow-collect.log` |
| 错误 | `.../flow-collect/error_flow-collect.log` |

> ⚠️ **无 fatal**。

### 串联查询字段：port、in_waterlevel/out_waterlevel。
...
> flow-collect 网络流量采集，数据主要上报/本地，**PG侧无专用业务表**。