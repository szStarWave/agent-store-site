...
| 用途 | 路径 |
|---|---|
| 主日志 | `/data/services/pcmgr_enterprise/logs/file-server/file-server.log` |
| 错误 | `.../file-server/error_file-server.log` |
| fatal | ⚠️ **在 server-configs 目录**：`.../logs/server-configs/fatal_file-server.log` |
...
> file-server 文件上传下载，文件存 COS，仅读 **system_config** 配置，**无业务主表**。