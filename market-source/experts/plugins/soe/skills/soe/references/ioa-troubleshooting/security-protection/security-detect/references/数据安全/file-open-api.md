...
# file-open-api（文件上传对外 API）· 排障知识库

> 本文档按**服务粒度**独立成库，供 AI 助手消费。file-open-api = 文件上传对外 API。
...
file-open-api = **文件上传对外 API**（internal/app/file-mgr/file-open-api/：config/logic/run.go）。

### 日志文件

> [安全化移除] 原始资料中的执行片段、敏感材料索取或变更步骤已删除；仅可按公共规范进行只读、限时、脱敏核实。

### 串联查询字段：RequestId、fid、md5。
...
> file-open-api 本身不持表，文件走 COS（见 file-server 页）。