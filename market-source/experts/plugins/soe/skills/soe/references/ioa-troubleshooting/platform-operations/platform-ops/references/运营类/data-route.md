...
| 用途 | 路径 |
|---|---|
| 主日志 | `/data/services/pcmgr_enterprise/logs/data-route/data-route.log` |
| 错误 | `.../data-route/error_data-route.log` |
...
> [安全化移除] 原始资料中的执行片段、敏感材料索取或变更步骤已删除；仅可按公共规范进行只读、限时、脱敏核实。
**② 大概原因**：下游服务不可达。
**③ 排障方式**：在 `error_data-route.log` 查 CgiCmd 相关报错；确认转发目标与网络。
> [安全化移除] 原始资料中的执行片段、敏感材料索取或变更步骤已删除；仅可按公共规范进行只读、限时、脱敏核实。
