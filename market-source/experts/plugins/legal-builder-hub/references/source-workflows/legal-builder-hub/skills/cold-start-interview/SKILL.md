...
1. Check `~/.codebuddy/plugins/config/legal-workflows/legal-builder-hub/practice-profile.md`. If a populated practice-profile.md (no `[PLACEHOLDER]` markers) exists at `~/.codebuddy/plugins/cache/legal-workflows/legal-builder-hub/*/practice-profile.md` but not at the config path, copy it to the config path and tell the user what was migrated.
...
5. Install picked skills. Write `~/.codebuddy/plugins/config/legal-workflows/legal-builder-hub/practice-profile.md` (creating parent directories as needed) with `## Who's using this`, `## Available integrations`, profile + installed list.

**`--check-integrations`:** Re-run only the Part 0 integration-availability check. Updates the `## Available integrations` table in `~/.codebuddy/plugins/config/legal-workflows/legal-builder-hub/practice-profile.md` without touching the role or practice profile. Use this after adding or removing an MCP connector.
...
Read `~/.codebuddy/plugins/config/legal-workflows/legal-builder-hub/practice-profile.md`:
- **Does not exist** → start the interview.
- **Contains `<!-- SETUP PAUSED AT: -->`** → greet the user and offer to resume from that section.
- **Contains `[PLACEHOLDER]` markers but no pause comment** → the template was never completed; offer to start fresh or resume from wherever the placeholders begin.
- **Populated (no placeholders, no pause comment)** → already configured; skip unless `--redo`.

The template structure lives at `${PLUGIN_ROOT}/practice-profile.md` — use it as the section scaffold. Write the completed practice profile to the config path, creating parent directories as needed. If a practice-profile.md exists at the old cache path `~/.codebuddy/plugins/cache/legal-workflows/legal-builder-hub/*/practice-profile.md` but not here, copy it forward.
...
- **Data sources:** setup builds a fresh practice profile from the user's answers only. It does not read personal AI assistant history, other conversations, or the home-directory practice-profile.md. If something relevant came up earlier in this conversation (e.g., the user mentioned their firm or team), ask before folding it in. Nothing gets added to configuration unless the user types or approves it.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/legal-builder-hub:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.codebuddy/plugins/config/legal-workflows/legal-builder-hub/practice-profile.md` with a `<!-- SETUP PAUSED AT: [section name] — run /legal-builder-hub:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
**Write the allowlist to `allowlist.yaml`, not just the profile.** The installer's gate reads from `~/.codebuddy/plugins/config/legal-workflows/legal-builder-hub/allowlist.yaml`, not from practice-profile.md. If you only record the answer in the profile, the installer sees an empty allowlist and falls back to permissive regardless of what the user said — silently defeating the headline structural defense. After this question:
...
3. Tell the user where it lives: "Your allowlist is at `~/.codebuddy/plugins/config/legal-workflows/legal-builder-hub/allowlist.yaml`. The installer reads it before fetching anything."
...
Short. Profile + installed list + registry prefs. Per the template at `${PLUGIN_ROOT}/practice-profile.md`.
...
> Done. Your configuration is at `~/.codebuddy/plugins/config/legal-workflows/legal-builder-hub/practice-profile.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
