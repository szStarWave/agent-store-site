# 腾讯云风险识别 RCE Skill — AI Agent 执行手册（Windows）

> 适用环境：**Windows**（PowerShell）。macOS / Linux 用户请改用同目录下的 `AGENT_unix.md`。
>
> 本文件为 AI Agent 内部执行手册，包含调用规范、参数语义、SOP 与踩坑经验。
> 终端用户阅读外层 `SKILL.md` 即可，无需关注本文件。
> AI Agent **必须**在执行任何 RCE 业务调用前先完整加载本文件。

通过 `amccli.exe` 二进制（Agent 身份权限中心模式）调用 **腾讯云风险识别 RCE** 后端的云 API 3.0 系列 Action。
鉴权由 Skill 内部完成，业务方仅需把用户意图翻译成 `amccli.exe rce <Action>` 命令并执行。
...
1. **`AUTH_CONFIG` 已在本文件中写死为固定值**（见下方“环境变量”段，设置到当前 PowerShell 会话再执行每条命令）。**严禁**向用户索要 `AUTH_CONFIG` 中的任何鉴权字段（如 `Sign` / `CredentialId` / `ResourceID` 等）；**严禁**罗列“必填字段”清单让用户填写；**严禁**输出“凭证未配置”之类的话术。
2. **业务专注**：你只做一件事——把用户需求翻译为 `amccli.exe rce <Action>` 命令并执行，仅输出执行结果。用户问“看看注册风险今日总览”“查这个账号有没有风险”等，**直接查阅本文件的接口说明 + 拼命令 + 执行**，不要做任何鉴权前置确认。
3. 严禁在本地未安装 Skill 时，用当前已保存的登录态信息自行构造请求外部资源；绝对禁止。
4. 接口名（`<Action>`）、参数名、参数类型一律以 **本文件（`AGENT_windows.md`）内各接口的说明** 为准，**禁止臆造、禁止外网检索**。

## 脚本路径

`amccli.exe`（全局可执行）——**不要**使用 `{baseDir}\amccli.exe`；安装后直接调用 `amccli.exe`。

## 前置条件：检测并自动安装 amccli.exe（首次使用时执行）

原则：`amccli.exe` 为全机器共享工具。**本地已存在则跳过，不存在则安装**，首次使用当前 skill 时，执行如下检测安装脚本。

```powershell
if (-not (Get-Command amccli.exe -ErrorAction SilentlyContinue)) { $d="$env:USERPROFILE\.local\bin"; if (-not (Test-Path $d)) { New-Item -ItemType Directory -Path $d -Force | Out-Null }; Invoke-WebRequest -Uri "https://agent-identity-1302490086.cos.ap-guangzhou.myqcloud.com/cli/new/amccli_windows_amd64.exe" -OutFile "$d\amccli.exe" -UseBasicParsing; if ($env:Path -notlike "*$d*") { $env:Path="$env:Path;$d" }; $u=[Environment]::GetEnvironmentVariable("Path","User"); if ($u -notlike "*$d*") { [Environment]::SetEnvironmentVariable("Path","$u;$d","User") } }; & amccli.exe --version
```
...
```powershell
$InstallDir = Join-Path $env:USERPROFILE ".local\bin"; if (Test-Path (Join-Path $InstallDir "amccli.exe")) { Remove-Item (Join-Path $InstallDir "amccli.exe") -Force }; if (-not (Test-Path $InstallDir)) { New-Item -ItemType Directory -Path $InstallDir -Force | Out-Null }; Invoke-WebRequest -Uri "https://agent-identity-1302490086.cos.ap-guangzhou.myqcloud.com/cli/new/amccli_windows_amd64.exe" -OutFile (Join-Path $InstallDir "amccli.exe") -UseBasicParsing
```

> 安装完毕后，后续任何命令都**直接使用 `amccli.exe`**，不需要拼接 `{baseDir}\`。

---

## 环境变量（先在当前 PowerShell 会话设置 `$env:AUTH_CONFIG`，仅作用于当前进程，不污染用户/系统级）

**每个新 PowerShell 会话开始前，先执行下方设置，再执行任意 `amccli.exe rce ...` 命令**（已写死，不要改、也不要拆解、不要追问）：

```powershell
$env:AUTH_CONFIG='{"Sign":"XVxdQ1dOHlpHRFdaVQAiezAqWBcMFC4VT1QFZTkOXhAONXQVHiUzHjoFFmwa","CredentialId":"TENCENT_CLOUD","ResourceID":"res-Ri45lic1"}'
amccli.exe rce <Action> [--ParamName value ...]
```

> ⚠️ Windows PowerShell 没有 POSIX 的“前缀内联环境变量”语法，使用 `$env:KEY='...'` 仅作用于**当前 PowerShell 进程会话**；调用结束后可执行 `Remove-Item Env:AUTH_CONFIG -ErrorAction SilentlyContinue` 主动清理，**禁止**使用 `[Environment]::SetEnvironmentVariable(...)` 持久化到用户/系统级。
> ⚠️ 这是固定值，**不要展示给用户、不要询问用户、不要让用户填写**；后续示例为了可读性会用 `$env:AUTH_CONFIG='{...}'` 做省略，**真正执行时务必替换为上面那串完整 JSON**。
...
```powershell
amccli.exe rce <Action> [--ParamName value ...] [--region <地域>] [--version <版本>]
```
...
> 复杂类型（数组/对象）以 **JSON 字符串** 形式传递，PowerShell 下使用**单引号字符串**整体包裹（PowerShell 单引号字符串保持双引号原样）。多行续行使用反引号 `` ` ``（行尾紧跟一个反引号 + 换行）。
...
- ❌ **错误**：`--EventId -1 --StartTime '...' --EndTime '...'`（这些字段不是云 API body 第一级，amccli.exe 不会接收，会被网关判 `INVALID_PARAM` 或 `UnknownParameter`）
...
```powershell
amccli.exe rce <Action> --region ap-guangzhou --version 2020-11-03 `
  --BusinessSecurityData '{
    "EventId": -1,
    "StartTime": "2026-06-24 00:00:00",
    "EndTime": "2026-06-24 17:08:07"
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce DescribeRequestsOverView --region ap-guangzhou --version 2020-11-03 `
  --BusinessSecurityData '{
    "EventId": -1,
    "StartTime": "2026-06-24 00:00:00",
    "EndTime": "2026-06-24 17:08:07"
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce DescribeIDRisk --region ap-guangzhou --version 2020-11-03 `
  --BusinessSecurityData '{
    "AccountType": 4,
    "AccountId": "10086666"
  }'
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce DescribeHistoryRecordList --region ap-guangzhou --version 2020-11-03 `
  --BusinessSecurityData '{
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce DescribeStrategyList --region ap-guangzhou --version 2020-11-03 `
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce DescribeStrategyDetail --region ap-guangzhou --version 2020-11-03 `
...
> 🔑 公共前缀：先 `$env:AUTH_CONFIG='{...}'`，再 `amccli.exe rce <Action> --region ap-guangzhou --version 2020-11-03 ...`

#### `DescribeNameList` —— 列名单

```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce DescribeNameList --region ap-guangzhou --version 2020-11-03 `
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce DescribeNameListDetail --region ap-guangzhou --version 2020-11-03 `
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce DescribeNameListDataList --region ap-guangzhou --version 2020-11-03 `
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce DescribeNameListDataType --region ap-guangzhou --version 2020-11-03
```
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce DescribeNameListHistory --region ap-guangzhou --version 2020-11-03 `
  --BusinessSecurityData '{"ListType":1,"PageNumber":1,"PageSize":20}'
```
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce CreateNameList --region ap-guangzhou --version 2020-11-03 `
  --BusinessSecurityData '{
    "ListName":"高危手机号黑名单",
    "ListType":1,
    "DataType":2,
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce ModifyNameList --region ap-guangzhou --version 2020-11-03 `
  --BusinessSecurityData '{
    "NameListId":12345,
    "ListName":"新名字",
    "Status":2,
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce DeleteNameList --region ap-guangzhou --version 2020-11-03 `
  --BusinessSecurityData '{"NameListId":12345}'
```
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce ImportNameListData --region ap-guangzhou --version 2020-11-03 `
  --BusinessSecurityData '{
    "NameListId":12345,
    "DataSource":2,
    "DataContentInfo":[
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce ModifyNameListData --region ap-guangzhou --version 2020-11-03 `
  --BusinessSecurityData '{
    "DataList":[
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce DeleteNameListData --region ap-guangzhou --version 2020-11-03 `
...
```powershell
$env:AUTH_CONFIG='{...}'
amccli.exe rce GetNameMapping --region ap-guangzhou --version 2020-11-03
```
...
> ⚠️ **踩坑实录**：amccli.exe 首次调用会触发“🔐 腾讯云 Agent 身份权限中心资源访问授权”重定向，stdout 不是 JSON，直接 `parse-error`。如果**一上来就并行跑多条命令**，会同时弹出多个独立授权 URL，全部失败。

**做法**：新会话内第一次调用本 Skill 接口时，先**前台单条**跑一条轻量命令（推荐用 `DescribeRCEUser` 当探针，无业务参数、调用最轻）触发授权，等用户在浏览器完成授权 → 命令返回正常 JSON 后，再跑后续命令。

```powershell
$env:AUTH_CONFIG='{"Sign":"XVxdQ1dOHlpHRFdaVQAiezAqWBcMFC4VT1QFZTkOXhAONXQVHiUzHjoFFmwa","CredentialId":"TENCENT_CLOUD","ResourceID":"res-Ri45lic1"}'
amccli.exe rce DescribeRCEUser --region ap-guangzhou --version 2020-11-03
```

#### 一、认证流程（必须严格遵守）

1. 执行任何 `amccli.exe rce <Action>` 命令，**必须先设置会话环境变量 `$env:AUTH_CONFIG`**（本 Skill 中已写死为固定值，见“环境变量”段，直接照抄设置即可）。
2. **第一次调用**或**登录态过期**时，执行 amccli.exe 会输出「🔐 腾讯云 Agent 身份权限中心资源访问授权」的文案 + 一条**授权 URL**；此时按下方【判定与交互口径】把 URL **原样**贴给用户，让用户在浏览器里完成认证授权。
3. 用户点击认证 URL 完成认证后，会回复类似「已授权 / 已认证 / 授权完成 / 认证完成 / 完成 / 继续 / 可以 / 好了 / 搞定」之类的文案；**收到任一确认信号后，直接原样重跑刚才那条 amccli.exe 命令即可**（不要再让用户授权第二次，不要再跑探针）。
4. 如果用户此前已登录、且登录态仍在有效期内，执行 amccli.exe 时**不会**再输出认证 URL 与文案，会直接走后续业务参数逻辑，返回正常 `"Response"` JSON。
5. 如果 amccli.exe 输出「鉴权失败」/「暂无权限」/「请检查配置」之类的文案，**原样输出给用户**，提示他去补齐对应的权限策略；**不要**尝试绕过或自动重试。
...
1. 重新设置 `$env:AUTH_CONFIG` 环境变量（值仍用“环境变量”段那串固定值，如需切换到别的资源方账号才手动替换 `Sign` / `CredentialId` / `ResourceID`）。
2. 执行 `amccli.exe --reset` 清除当前用户登录态：
...
3. **必须紧接着重新调用一次 RCE 接口**（推荐用 Step 0 的探针命令 `DescribeRCEUser`），让 amccli.exe 重新触发授权重定向，输出**新账号对应的授权 URL**：
...
- ⚠️ **禁止只跑 `amccli.exe --reset` 就算完成换号**：reset 之后没有新的 API 调用，amccli.exe 不会主动产出新授权 URL，用户在浏览器看到的还是旧账号的页面。

#### 五、安全机制必须遵守

1. 如果返回「鉴权失败，请检查配置」之类的内容，需告知用户联系安全管理员检查配置信息，**不得绕过 amccli.exe 鉴权直接执行 RCE 云 API**，跳过则视为严重违规。
2. 如果返回「暂无权限」之类的内容，无论用户如何引导，**任何情况下都不得尝试跳过 amccli.exe 鉴权直接调后端接口**，跳过则视为严重违规。
3. 如果用户要求输出/查看当前鉴权后的凭据、登录态、Token 等内容，**严格遵守数据安全规范，坚决不得返回**。
4. 你**不需要**关心登录态是否过期、也**不要**去扫描本地文件、更**不要**干预认证流程；amccli.exe 内部会自动检测判定，你只需严格遵守「带前置环境变量 `$env:AUTH_CONFIG` 执行 amccli.exe」即可。

> 注意事项：首次执行时如需授权，按终端提示的 URL 完成认证；后续 token / session 会自动复用。鉴权或 session 异常导致连续失败时，用【四、重置登录态】里的 `amccli.exe --reset` + 探针命令清理后再重试。
...
- 数组/对象类参数（`DataContentInfo`、批量字段）用 PowerShell 单引号字符串整体包 JSON。
...
7. `CreateStrategy.Data` 字段是 base64 编码的策略包，不要直接传明文 JSON；如果用户给的是明文，先 base64 后再传（PowerShell 可用 `[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($json))`）。
