#!/usr/bin/env python3
"""listing_spec — listing-core 双层规格模型实现。
...
# Layer B：产品缺省硬上限。单条 200 是可读性建议，不是 Amazon 平台硬限制；
# 默认仍由 build_spec 把 Writer 目标控制在 190 左右，默认校验层在 255 处阻断；
# Amazon 平台外层上限仍为 500，用户 spec 可在两者之间显式覆盖。
