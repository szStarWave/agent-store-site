...
session-id 取自环境变量 SESSION_ID；不存在时自动生成 `HHMMSS-<6 hex>`。
...
def writable_root() -> str:
    """返回一个真实可写的绝对路径根目录，供缓存/输出使用。

    候选顺序：$LINKFOX_WORKSPACE → os.getcwd() → tempfile.gettempdir()。
    每个候选都做真实探针（在候选目录下创建 .lfx-probe 子目录再删除），
    仅 os.access 检查在部分只读挂载（如生产环境的 skill 包只读挂载）下不可靠，
    因此不能替代真实创建/删除的探测。找不到任何可写候选时兜底返回
    tempfile.gettempdir()（即使探针失败也返回，避免抛异常）。
    """
    import tempfile

    candidates = []
    env = (os.environ.get("LISTING_WORKSPACE") or os.environ.get("LINKFOX_WORKSPACE") or "").strip()
    if env:
    ...
    for candidate in candidates:
        ...
        probe = os.path.join(abs_candidate, ".lfx-probe")
        try:
...
def _linkfox_root() -> str:
    ...
    优先 $ACPX_WORKSPACES；这是 ACP 文件服务和前端最稳定可见的位置。当该目录不可写时，
    依次回退到当前工作目录、~/linkfox，最后退到系统临时目录。选定结果在进程内缓存，
    保证同一次运行内所有落盘路径稳定一致。
    """
    ...
    for root in candidates:
        try:
            ...
            probe = os.path.join(root, f".write_probe.{os.getpid()}.{secrets.token_hex(2)}")
        ...
        return root
    _SESSION_CACHE["_root"] = candidates[-1]
    return candidates[-1]
...
def _timestamp_suffix(ts: float) -> str:
    """用于文件名的并发安全后缀，避免多进程同一微秒落盘互相覆盖。"""
    return f"{int(ts * 1_000_000)}-{os.getpid()}-{secrets.token_hex(2)}"


@contextmanager
def _locked_metadata(path: str):
    """Serialize cross-process updates to a session metadata file."""
...
def _ensure_meta(root: str, session_dir: str, date_str: str, sid: str, ts: float) -> None:
    ...
    with _locked_metadata(meta_path):
...
def _update_meta(session_dir: str, *, skill: str, kind: str, file_rel: str, ts: float) -> None:
    ...
    with _locked_metadata(meta_path):
        ...
        meta.setdefault(bucket, []).append(file_rel)
...
def resolve_data_path(slug: str, ts: float, ext: str = "json") -> str:
    ...
    out = os.path.join(sub, f"{slug}-{_timestamp_suffix(ts)}.{ext}")
...
def download_media(url: str, slug: str, ts: Optional[float] = None, ext: Optional[str] = None, timeout: int = 300) -> Optional[str]:
    ...
    try:
        ...
        final_path = os.path.join(media_dir, f"{slug}-{ts_us}.{guessed_ext}")
