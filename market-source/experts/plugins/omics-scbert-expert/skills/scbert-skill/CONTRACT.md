...
scbert-skill 通过 wrapper（`scripts/omics_cli.py`）调用 CLI；wrapper 在 argparse 层物理上注册 8 条一级命令，
但本 SKILL **只允许**调用以下子集：
...
| 5   | `omics list apps`                 | `--type <AppType>`                        | read           | ✅ **仅限"导入前同名检查"用途；必须带 `--type NEXTFLOW`**；其它形态禁用                                                              |
| 6   | `omics cos upload`                | `--file <path> --cos-path <path>`      | write COS     | ✅ **用于上传本地生成的文件到 COS**（见 SKILL.md §COS 上传流程）                                    |
| 7   | `omics run`                       | —                                      | 写远端任务   | ✅ **必须先二次确认；`--public-app` `--public-app` 固定为 `0de1b4ae-1543-4882-8bae-b5ee87968bc5`**                    |
| 8   | `omics status`                    | —                                      | read           | ✅                                                                                                 |
