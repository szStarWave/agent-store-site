...
生成的产物 SKILL 只能调用以下命令，禁止越界：
    ...
    login / whoami / quota / config / list（region / project / env / cos-bucket / apps）
...
class OmicsCLI:
    ...
    def build_quota(self, output: str = "table") -> list[str]:
        ...
        SKILL 调用时机：Step 0.3 首检 与 Step 4.5 运行前检查（仅 C端用户）。
        """
    ...
    def build_run(
        ...
        """
        omics run（独立应用模式）：
        ⚠️ --public-app 固定为 '0de1b4ae-1543-4882-8bae-b5ee87968bc5'，不接受替换。
        ⚠️ AppType 固定为 'NEXTFLOW'，以 --app-type 传给 CLI。
        """
        PUBLIC_APP_ID = '0de1b4ae-1543-4882-8bae-b5ee87968bc5'  # 硬编码，禁止覆盖
        APP_TYPE = 'NEXTFLOW'      # 硬编码，以 --app-type 传给 CLI
        
        if not nf_version:
            raise ValueError(
                "本应用（0de1b4ae-1543-4882-8bae-b5ee87968bc5）为 NEXTFLOW 类型，必须指定 nf_version"
            )
...
def main():
    ...
    # 2.1 quota（C端体验配额查询）
    quota_p = subparsers.add_parser("quota", help="查询C端体验配额（剩余天数/今日剩余次数）")
