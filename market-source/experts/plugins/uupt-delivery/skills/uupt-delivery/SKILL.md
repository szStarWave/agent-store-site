...
| 参数 | 说明 | 必填 |
|------|------|------|
| `--source` | 领取来源（决定可领哪些券包，默认 2，一般无需传入） | 否 |
...
配置分为两层，优先级：**环境变量 > config.json > defaults.json > 内置默认值**。

| 文件 | 内容 | 说明 |
|------|------|------|
| `defaults.json`（skill 目录） | appId、apiUrl | 内置凭证，**请勿修改** |
| `~/.uupt-delivery/config.json` | openId | 手机号注册成功后自动生成，保存在用户主目录 |

### 环境变量

| 变量 | 说明 |
|------|------|
| `UUPT_APP_ID` | 应用 ID（可选覆盖） |
| `UUPT_OPEN_ID` | 用户唯一标识 |
...
// 领取优惠券
const coupon = await receiveCouponPackages({ source: 2 });
```
...
# 领取优惠券
coupon = receive_coupon_packages(source=2)
```
