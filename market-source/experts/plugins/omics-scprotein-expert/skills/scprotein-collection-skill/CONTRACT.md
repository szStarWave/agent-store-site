...
scprotein-collection-skill 通过 wrapper（`scripts/omics_cli.py`）调用 CLI；wrapper 在 argparse 层物理上注册 8 条一级命令，
但本 SKILL **只允许**调用以下子集：
...
| 4   | `omics list public-apps`          | `--parent-app <CollectionAppId>`（合集展开） | read           | ⏭️ **本 SKILL 已硬编码子应用清单，运行时不调用此命令；CLI 仍保留此能力供元 SKILL 生成时使用** |
