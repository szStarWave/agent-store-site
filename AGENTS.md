...
| 路径 | 角色 |
| --- | --- |
| `src/` | 应用代码：`pages/`（路由）、`views/`（页面组件）、`components/`、`theme/`（swizzle：`Root` + `Footer` 与 `NavbarItem` 的薄包装）、`i18n/`、`lib/`、`plugins/`、`remark/`、`css/`（`tokens.css` 设计令牌全局 / `style.css` 站点组件 / `docusaurus-overrides.css` 全站外壳，无 Tailwind / UnoCSS） |
